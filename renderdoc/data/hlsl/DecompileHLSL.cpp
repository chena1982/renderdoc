...
// I changed all the variants that are simple and clear, and left the unusual, less clear variants
// as is. Changing to size_t fixes a large number of signed/unsigned mismatch warnings.
//
// We are also using the CRT_SECURE flags to automatically fix sprintf, sscanf into _s safe versions
// where it can. _CRT_SECURE_CPP_OVERLOAD_STANDARD_NAMES=1
...
#pragma warning(disable : 6328)
  ...
  Decompiler() : mLastStatement(0), uuidVar(0), nestCount(0) {}
  ...
  {
    if(!strcmp(name, "float4x4"))
      return DT_float4x4;
    if(!strcmp(name, "float4x3"))
      return DT_float4x3;
    if(!strcmp(name, "float4x2"))
      return DT_float4x2;
    if(!strcmp(name, "float2x4"))
      return DT_float2x4;
    if(!strcmp(name, "float3x4"))
      return DT_float3x4;
    if(!strcmp(name, "float4"))
      return DT_float4;
    if(!strcmp(name, "float3x3"))
      return DT_float3x3;
    if(!strcmp(name, "float3"))
      return DT_float3;
    if(!strcmp(name, "float2"))
      return DT_float2;
    if(!strcmp(name, "float1"))
      return DT_float;
    if(!strcmp(name, "float"))
      return DT_float;
    if(!strcmp(name, "bool"))
      return DT_bool;
    if(!strcmp(name, "uint4"))
      return DT_uint4;
    if(!strcmp(name, "uint3"))
      return DT_uint3;
    if(!strcmp(name, "uint2"))
      return DT_uint2;
    if(!strcmp(name, "uint"))
      return DT_uint;
    if(!strcmp(name, "int4"))
  ...
  void ParseInputSignature(const char *c, size_t size)
  {
    ...
    while(pos < size)
    {
      ...
      if(!strncmp(c + pos, "// no Input", strlen("// no Input")))
        break;
      int numRead = sscanf_s(c + pos, "// %s %d %s %d %s %s", name, 256, &index, mask, 16, &slot,
                             format2, 16, format, 16);
      if(numRead != 6)
      {
  ...
  void ParseOutputSignature(const char *c, size_t size)
  {
    ...
    while(pos < size)
    {
      ...
      if(numRead == 6)
      {
        ...
        // Write.
        char buffer[256];
        sprintf_s(buffer, 256, "  out %s %s : %s%d,\n", format2, regNameStr.c_str(), name, index);
        mOutput.insert(mOutput.end(), buffer, buffer + strlen(buffer));
      ...
      {
        char sysValue[64];
        sscanf_s(c + pos, "// %s %d %s %s %s %s", name, 256, &index, mask, 16, sysValue, 64,
                 format2, 16, format, 16);
        // Write.
        char buffer[256];
        sprintf_s(buffer, 256, "  out %s %s : %s,\n", format, sysValue, name);
        mOutput.insert(mOutput.end(), buffer, buffer + strlen(buffer));
      }
      if(numRead != 6)
      {
  ...
  void ReadResourceBindings(const char *c, size_t size)
  {
    ...
    while(pos < size)
    {
      ...
      int slot, arraySize;
      type[0] = 0;
      int numRead = sscanf_s(c + pos, "// %s %s %s %s %d %d", name, 256, type, 16, format, 16, dim,
                             16, &slot, &arraySize);
      if(numRead != 6)
        logDecompileError("Error parsing resource declaration: " + string(c + pos, 80));
      if(!strcmp(type, "sampler"))
      {
        char *escapePos = strchr(name, '[');
        if(escapePos)
          *escapePos = '_';
        escapePos = strchr(name, ']');
        if(escapePos)
          *escapePos = '_';
        ...
        if(arraySize > 1)
          ...
          {
            sprintf_s(name, 256, "%s[%d]", baseName.c_str(), i);
      ...
      else if(!strcmp(type, "texture"))
      {
        ...
        {
          // The documentation says it's not legal, but we see Texture 2DMS with no ending size in
          // WatchDogs. Instead of "2dMS4", it's just "2dMS". If we set that to Texture2DMS<float4>,
          // with no size, it generates the same code as the ASM.	 sscanf returns -1 as an error.
          //
          ...
          if(scanned == 1)
            sprintf_s(buffer, 256, "Texture2DMS<%s,%d>", format, msnumber);
          else
            sprintf_s(buffer, 256, "Texture2DMS<%s>", format);
  ...
  void WriteResourceDefinitions()
  {
    ...
    for(map<int, string>::iterator i = mSamplerNames.begin(); i != mSamplerNames.end(); ++i)
    {
      ...
      {
        sprintf_s(buffer, 256, "SamplerState %s : register(s%d);\n", i->second.c_str(), i->first);
        /*
      ...
      else if(mSamplerNamesArraySize[i->first] > 1)
      {
        ...
        sprintf_s(buffer, 256, "SamplerState %s[%d] : register(s%d);\n", baseName.c_str(),
                  mSamplerNamesArraySize[i->first], i->first);
        mOutput.insert(mOutput.end(), buffer, buffer + strlen(buffer));
      }
    }
    for(map<int, string>::iterator i = mSamplerComparisonNames.begin();
        i != mSamplerComparisonNames.end(); ++i)
    {
      if(mSamplerComparisonNamesArraySize[i->first] == 1)
      {
        sprintf_s(buffer, 256, "SamplerComparisonState %s : register(s%d);\n", i->second.c_str(),
                  i->first);
        /*
      ...
      {
        string baseName = i->second.substr(0, i->second.find('['));
        sprintf_s(buffer, 256, "SamplerComparisonState %s[%d] : register(s%d);\n", baseName.c_str(),
                  mSamplerComparisonNamesArraySize[i->first], i->first);
        mOutput.insert(mOutput.end(), buffer, buffer + strlen(buffer));
      }
    }
    for(map<int, string>::iterator i = mTextureNames.begin(); i != mTextureNames.end(); ++i)
    {
      ...
      {
        sprintf_s(buffer, 256, "%s %s : register(t%d);\n", mTextureType[i->first].c_str(),
                  i->second.c_str(), i->first);
        mOutput.insert(mOutput.end(), buffer, buffer + strlen(buffer));
      }
      ...
      {
        string baseName = i->second.substr(0, i->second.find('['));
        sprintf_s(buffer, 256, "%s %s[%d] : register(t%d);\n", mTextureType[i->first].c_str(),
                  baseName.c_str(), mTextureNamesArraySize[i->first], i->first);
        mOutput.insert(mOutput.end(), buffer, buffer + strlen(buffer));
      }
    }
  }
  ...
  int getDataTypeSize(DataType d)
  {
    switch(d)
    {
      ...
      case DT_float2:
      case DT_uint2:
      case DT_int2: return 8;
      case DT_float3:
      case DT_uint3:
      case DT_int3: return 12;
      case DT_float4:
      case DT_uint4:
      case DT_int4: return 16;
      case DT_float2x4:
      case DT_float4x2: return 32;
      case DT_float3x3: return 36;
      case DT_float3x4: return 48;
      case DT_float4x3: return 48;
      case DT_float4x4: return 64;
  ...
  void ParseBufferDefinitions(Shader *shader, const char *c, size_t size)
  {
    ...
    while(pos < size - strlen(headerid))
    {
      ...
      if(pos >= size - strlen(headerid))
        return;
      char name[256];
      int numRead = sscanf_s(c + pos, "// cbuffer %s", name, 256);
      if(numRead != 1)
      {
      ...
      map<string, int>::iterator it = mCBufferNames.find(name);
      if(it == mCBufferNames.end())
      {
        logDecompileError("Buffer not found in resource declaration: " + string(name));
        return;
      }
      const int bufferRegister = it->second;
      // Write declaration.
      char buffer[256];
      if(name[0] == '$')
        name[0] = '_';
      ...
      do
      {
        ...
        {
          int ePos = 2;
          while(buffer[ePos] != 0 && (buffer[ePos] == ' ' || buffer[ePos] == '\t' ||
                                      buffer[ePos] == '\n' || buffer[ePos] == '\r'))
            ++ePos;
          if(!buffer[ePos])
          {
        ...
        if(strstr(buffer, " }"))
        {
          ...
          else
          {
            ...
            {
              sprintf_s(buffer, 256, "%s[%d].", structName.c_str(), i);
              for(vector<int>::iterator j = pendingStructAttributes[structLevel].begin();
                  j != pendingStructAttributes[structLevel].end(); ++j)
              {
        ...
        char type[16];
        type[0] = 0;
        numRead = sscanf_s(c + pos, "// %s %s", type, 16, name, 256);
        if(numRead != 2)
        {
        ...
        if(!strcmp(type, "row_major") || !strcmp(type, "column_major"))
        {
          ...
          modifier = type;
          modifier.push_back(' ');
          numRead = sscanf_s(c + pos, "// %s %s %s", buffer, 256, type, 16, name, 256);
          if(numRead != 3)
          {
            logDecompileError("Error parsing buffer item: " + string(c + pos, 80));
            return;
        ...
        */
        ...
        // This section previously made a distinction for the floatYxZ formats, assuming that they
        // were not able to be arrays.  That's untrue, so they were added into the for loop to
        // create the array elements possible. This fixes a series of array out of bounds errors for
        // anything using these floatYxZ matrices.

        // Look for array based elements.  We need an mCBufferData element for each possible array
        // element, to match possible uses of those offsets in the ASM code.
        size_t ep = e.Name.find('[');
        if(ep != string::npos &&
           (e.bt == DT_bool || e.bt == DT_float || e.bt == DT_float2 || e.bt == DT_float3 ||
            e.bt == DT_float4 || e.bt == DT_uint || e.bt == DT_uint2 || e.bt == DT_uint3 ||
            e.bt == DT_uint4 || e.bt == DT_int || e.bt == DT_int2 || e.bt == DT_int3 ||
            e.bt == DT_int4 || e.bt == DT_float4x4 || e.bt == DT_float3x4 || e.bt == DT_float4x3 ||
            e.bt == DT_float3x3 || e.bt == DT_float4x2 || e.bt == DT_float2x4))
        {
          ...
          int counter = 0;
          if(e.bt == DT_float || e.bt == DT_bool || e.bt == DT_uint || e.bt == DT_int)
            counter = 4;
          else if(e.bt == DT_float2 || e.bt == DT_uint2 || e.bt == DT_int2)
            counter = 8;
          else if(e.bt == DT_float3 || e.bt == DT_uint3 || e.bt == DT_int3)
            counter = 12;
          else if(e.bt == DT_float4 || e.bt == DT_uint4 || e.bt == DT_int4)
            counter = 16;
          else if(e.bt == DT_float4x2 || e.bt == DT_float2x4)
            counter = 16 * 2;
          else if(e.bt == DT_float3x3 || e.bt == DT_float4x3)
            counter = 16 * 3;
          else if(e.bt == DT_float3x4 || e.bt == DT_float4x4)
            counter = 16 * 4;
          ...
          {
            sprintf_s(buffer, 256, "%s[%d]", baseName.c_str(), i);
            e.Name = buffer;
            int offsetPos;
                ...
                e.bt == DT_float2x4 || e.bt == DT_float3x4 || e.bt == DT_float4x4)
        {
          ...
          if(structLevel >= 0)
            pendingStructAttributes[structLevel].push_back(offsetPos);
          e.matrixRow = 1;
          offsetPos = (bufferRegister << 16) + offset + 1 * 16;
          mCBufferData[offsetPos] = e;
          if(structLevel >= 0)
            pendingStructAttributes[structLevel].push_back(offsetPos);
          if(e.bt != DT_float4x2)
          {
            e.matrixRow = 2;
            offsetPos = (bufferRegister << 16) + offset + 2 * 16;
            mCBufferData[offsetPos] = e;
            if(structLevel >= 0)
              pendingStructAttributes[structLevel].push_back(offsetPos);
            ...
            {    // Nearly sure this missing nesting was a bug
              e.matrixRow = 3;
              offsetPos = (bufferRegister << 16) + offset + 3 * 16;
              mCBufferData[offsetPos] = e;
              if(structLevel >= 0)
                pendingStructAttributes[structLevel].push_back(offsetPos);
            }
        ...
        int packoffset = offset / 16;
        int suboffset = (offset % 16) / 4;
        const char INDEX_MASK[] = "xyzw";
        ...
        if(!strncmp(c + pos, defaultid, strlen(defaultid)))
        {
          ...
          if(e.bt == DT_bool)
          {
            ...
            {
              if(suboffset == 0)
                sprintf_s(buffer, 256, "  %s%s %s : packoffset(c%d) = %s;\n", modifier.c_str(),
                          type, name, packoffset, bString.c_str());
              else
                sprintf_s(buffer, 256, "  %s%s %s : packoffset(c%d.%c) = %s;\n", modifier.c_str(),
                          type, name, packoffset, INDEX_MASK[suboffset], bString.c_str());
            }
            else
              sprintf_s(buffer, 256, "  %s%s%s %s = %s;\n", structSpacing.c_str(), modifier.c_str(),
                        type, name, bString.c_str());
            mOutput.insert(mOutput.end(), buffer, buffer + strlen(buffer));
            while(c[pos] != 0x0a && pos < size)
              pos++;
            pos++;
          ...
          else
          {
            ...
            if(useInt)
              ...
              numRead = sscanf_s(c + pos, "// = 0x%lx 0x%lx 0x%lx 0x%lx;", (unsigned long *)(v + 0),
                                 (unsigned long *)(v + 1), (unsigned long *)(v + 2),
                                 (unsigned long *)(v + 3));

            if(structLevel < 0)
            {
              if(suboffset == 0)
                sprintf_s(buffer, 256, "  %s%s %s : packoffset(c%d) = %s(", modifier.c_str(), type,
                          name, packoffset, type);
              else
                sprintf_s(buffer, 256, "  %s%s %s : packoffset(c%d.%c) = %s(", modifier.c_str(),
                          type, name, packoffset, INDEX_MASK[suboffset], type);
            }
            else
              sprintf_s(buffer, 256, "  %s%s%s %s = %s(", structSpacing.c_str(), modifier.c_str(),
                        type, name, type);
            mOutput.insert(mOutput.end(), buffer, buffer + strlen(buffer));
            ...
            {
              if(useInt)
                sprintf_s(buffer, 256, "%i,", in[i]);
              else
                sprintf_s(buffer, 256, "%e,", v[i]);
              mOutput.insert(mOutput.end(), buffer, buffer + strlen(buffer));
            }
            if(useInt)
              sprintf_s(buffer, 256, "%i);\n", in[numRead - 1]);
            else
              sprintf_s(buffer, 256, "%e);\n", v[numRead - 1]);
        ...
        else
        {
          ...
          {
            if(suboffset == 0)
              sprintf_s(buffer, 256, "  %s%s %s : packoffset(c%d);\n", modifier.c_str(), type, name,
                        packoffset);
            else
              sprintf_s(buffer, 256, "  %s%s %s : packoffset(c%d.%c);\n", modifier.c_str(), type,
                        name, packoffset, INDEX_MASK[suboffset]);
          }
          else
            sprintf_s(buffer, 256, "  %s%s%s %s;\n", structSpacing.c_str(), modifier.c_str(), type,
                      name);
          mOutput.insert(mOutput.end(), buffer, buffer + strlen(buffer));
        }
  ...
  void applySwizzle(const char *left, char *right, bool useInt = false)
  {
    ...
    // find the first dot in the string.  That's not right for left side array indices, so I changed
    // it to look for the far right dot instead.  Should be correct, but this is used everywhere.
    ...
    if(right[0] == 'l')
    {
      ...
      else
      {
        ...
        if(pos == 1)
        {
          sprintf_s(right2, opcodeSize, "%.6f", args[idx[0]]);
        }
        else
        {
          // Only integer values?
          ...
          if(isInt && useInt)
          {
            sprintf_s(right2, opcodeSize, "int%zu(", pos);
            for(int i = 0; idx[i] >= 0 && i < 4; ++i)
              sprintf_s(right2 + strlen(right2), sizeof(right2) - strlen(right2), "%d,",
                        int(args[idx[i]]));
            right2[strlen(right2) - 1] = 0;
          ...
          {
            sprintf_s(right2, opcodeSize, "float%zu(", pos);
            for(int i = 0; idx[i] >= 0 && i < 4; ++i)
              sprintf_s(right2 + strlen(right2), sizeof(right2) - strlen(right2), "%.6f,",
                        args[idx[i]]);
            right2[strlen(right2) - 1] = 0;
            strcat_s(right2, opcodeSize, ")");
          }
    ...
    else if(right[0] == 'c' && right[1] != 'b')    // dx9的常量寄存器，c开头
    {
      ...
      strncpy_s(right2, opcodeSize, right, strPos - right);
      ...
      char *cpos = strchr(right2, '.');
      if(cpos != NULL)
      {
        strncpy_s(buff, opcodeSize, right2, cpos - right2);
        buff[cpos - right2] = 0;
        size_t len = strlen(right2) - (right2 - cpos) - 1;
        strncpy_s(suffix, opcodeSize, cpos + 1, len);
        suffix[len] = 0;
      }
      else
      {
    ...
    else
    {
      ...
      if(strPos)
      {
        ...
        if(sscanf_s(strPos, "cb%d[%[^+]+%d]", &bufIndex, regAndSwiz, opcodeSize, &bufOffset) == 3)
        {
          // Some constant buffers no longer have variable names, giving us generic names like
          // cb0[23]. The syntax doesn't work to use those names, so in this scenario, we want to
          // just use the strPos name, unchanged.
        ...
        // Like: cb0[r0.w].xy
        else if(sscanf_s(strPos, "cb%d[%s]", &bufIndex, regAndSwiz, opcodeSize) == 2)
        {
          bufOffset = 0;
        }
        // Like: icb[r0.w+0].xyzw
        else if(sscanf_s(strPos, "cb[%[^+]+%d]", regAndSwiz, opcodeSize, &bufOffset) == 2)
        {
        ...
        // Like: icb[r1.z].xy
        else if(sscanf_s(strPos, "cb[%s]", regAndSwiz, opcodeSize) == 1)
        {
          bufIndex = -1;    // -1 is used as 'index' for icb entries.
          bufOffset = 0;
        }
        ...
        if((i->second.bt == DT_float || i->second.bt == DT_uint || i->second.bt == DT_int) ||
           ((i->second.bt == DT_float2 || i->second.bt == DT_uint2 || i->second.bt == DT_int2) &&
            (strrchr(right2, '.')[1] == 'w' || strrchr(right2, '.')[1] == 'z')) ||
           ((i->second.bt == DT_float3 || i->second.bt == DT_uint3 || i->second.bt == DT_int3) &&
            strrchr(right2, '.')[1] == 'w'))
        {
          int skip = 4;
          if(i->second.bt == DT_float || i->second.bt == DT_uint || i->second.bt == DT_int)
            skip = 1;
          else if(i->second.bt == DT_float2 || i->second.bt == DT_uint2 || i->second.bt == DT_int2)
            skip = 2;
          else if(i->second.bt == DT_float3 || i->second.bt == DT_uint3 || i->second.bt == DT_int3)
            skip = 3;
          char *dotPos = strrchr(right2, '.');
          int lowOffset = dotPos[1] - 'x';
          if(dotPos[1] == 'w')
            lowOffset = 3;
          if(lowOffset >= skip)
          {
        ...
        right3[0] = 0;
        strcat_s(right3, opcodeSize, i->second.Name.c_str());
        ...
        if(regAndSwiz[0])
        {
          ...
          if(isCorrected != mCorrectedIndexRegisters.end())
          {
            ...
            strcpy_s(newOperand, opcodeSize, isCorrected->second.c_str());
            ...
            sprintf_s(right3 + strlen(right3), sizeof(right3) - strlen(right3), "[%s]", newOperand);
                  ...
                  (i->second.bt == DT_float4x4 || i->second.bt == DT_float4x3 ||
                   i->second.bt == DT_float4x2 || i->second.bt == DT_float2x4 ||
                   i->second.bt == DT_float3x4 || i->second.bt == DT_float3x3))
          {
            char newOperand[opcodeSize];
            strcpy_s(newOperand, opcodeSize, mMulOperand.c_str());
            applySwizzle(regAndSwiz, newOperand, true);
            sprintf_s(right3 + strlen(right3), sizeof(right3) - strlen(right3), "[%s]", newOperand);
          ...
          else
          {
            ...
            size_t leftPos = base.find('[') + 1;
            size_t length = base.find(']') - leftPos;
            base.replace(leftPos, length, regAndSwiz);
            strcpy_s(right3, opcodeSize, base.c_str());
          }
        }
        if(i->second.bt != DT_float && i->second.bt != DT_bool && i->second.bt != DT_uint &&
           i->second.bt != DT_int)
        {
          strcat_s(right3, opcodeSize, ".");
          if(i->second.bt == DT_float4x4 || i->second.bt == DT_float4x3 ||
             i->second.bt == DT_float4x2 || i->second.bt == DT_float2x4 ||
             i->second.bt == DT_float3x4 || i->second.bt == DT_float3x3)
          {
            ...
            while(*++strPos)
            {
              ...
              {
                case 'x':
                  sprintf_s(right3 + strlen(right3), sizeof(right3) - strlen(right3),
                            i->second.isRowMajor ? "_m%d0" : "_m0%d", i->second.matrixRow);
                  break;
                case 'y':
                  sprintf_s(right3 + strlen(right3), sizeof(right3) - strlen(right3),
                            i->second.isRowMajor ? "_m%d1" : "_m1%d", i->second.matrixRow);
                  break;
                case 'z':
                  sprintf_s(right3 + strlen(right3), sizeof(right3) - strlen(right3),
                            i->second.isRowMajor ? "_m%d2" : "_m2%d", i->second.matrixRow);
                  break;
                case 'w':
                  sprintf_s(right3 + strlen(right3), sizeof(right3) - strlen(right3),
                            i->second.isRowMajor ? "_m%d3" : "_m3%d", i->second.matrixRow);
                  break;
          ...
          {
            strPos = strrchr(right2, '.');
            strcat_s(right3, opcodeSize, strPos + 1);
          }
        }
        strcpy_s(right2, opcodeSize, right3);
      }
    }
    if(absolute && negative)
      sprintf_s(right, opcodeSize, "-abs(%s)", right2);
  ...
  int ReadStatement(const char *pos)
  {
    ...
    strncpy_s(lineBuffer, 256, pos, 255);
    lineBuffer[255] = 0;
    ...
    int numRead = sscanf_s(lineBuffer, "%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s", statement,
                           128, op1, opcodeSize, op2, opcodeSize, op3, opcodeSize, op4, opcodeSize,
                           op5, opcodeSize, op6, opcodeSize, op7, opcodeSize, op8, opcodeSize, op9,
                           opcodeSize, op10, opcodeSize, op11, opcodeSize, op12, opcodeSize, op13,
                           opcodeSize, op14, opcodeSize, op15, opcodeSize);

    // Was previously a subroutine to CollectBrackets, but generated a lot of warnings, so putting
    // it inline allows the automatic CRT_SECURE macros to find the sizes.
    //  CollectBrackets(op1, op2, op3, op4, op5, op6, op7, op8, op9, op10, op11, op12, op13, op14,
    //  op15);
    {
      if(!strncmp(op1, "l(", 2) && op1[strlen(op1) - 1] != ')' && op1[strlen(op1) - 2] != ')')
      {
        strcat_s(op1, opcodeSize, " ");
        strcat_s(op1, opcodeSize, op2);
        strcat_s(op1, opcodeSize, " ");
        strcat_s(op1, opcodeSize, op3);
        strcat_s(op1, opcodeSize, " ");
        strcat_s(op1, opcodeSize, op4);
        strcpy_s(op2, opcodeSize, op5);
        strcpy_s(op3, opcodeSize, op6);
        strcpy_s(op4, opcodeSize, op7);
        strcpy_s(op5, opcodeSize, op8);
        strcpy_s(op6, opcodeSize, op9);
        strcpy_s(op7, opcodeSize, op10);
        strcpy_s(op8, opcodeSize, op11);
        strcpy_s(op9, opcodeSize, op12);
        strcpy_s(op10, opcodeSize, op13);
        strcpy_s(op11, opcodeSize, op14);
        strcpy_s(op12, opcodeSize, op15);
        op13[0] = 0;
        op14[0] = 0;
        op15[0] = 0;
      }
      ...
      {
        strcat_s(op2, opcodeSize, " ");
        strcat_s(op2, opcodeSize, op3);
        strcat_s(op2, opcodeSize, " ");
        strcat_s(op2, opcodeSize, op4);
        strcat_s(op2, opcodeSize, " ");
        strcat_s(op2, opcodeSize, op5);
        strcpy_s(op3, opcodeSize, op6);
        strcpy_s(op4, opcodeSize, op7);
        strcpy_s(op5, opcodeSize, op8);
        strcpy_s(op6, opcodeSize, op9);
      ...
      {
        strcat_s(op3, opcodeSize, " ");
        strcat_s(op3, opcodeSize, op4);
        strcat_s(op3, opcodeSize, " ");
        strcat_s(op3, opcodeSize, op5);
        strcat_s(op3, opcodeSize, " ");
        strcat_s(op3, opcodeSize, op6);
        strcpy_s(op4, opcodeSize, op7);
        strcpy_s(op5, opcodeSize, op8);
        strcpy_s(op6, opcodeSize, op9);
        strcpy_s(op7, opcodeSize, op10);
      ...
      {
        strcat_s(op4, opcodeSize, " ");
        strcat_s(op4, opcodeSize, op5);
        strcat_s(op4, opcodeSize, " ");
        strcat_s(op4, opcodeSize, op6);
        strcat_s(op4, opcodeSize, " ");
        strcat_s(op4, opcodeSize, op7);
        strcpy_s(op5, opcodeSize, op8);
        strcpy_s(op6, opcodeSize, op9);
        strcpy_s(op7, opcodeSize, op10);
        strcpy_s(op8, opcodeSize, op11);
      ...
      {
        strcat_s(op5, opcodeSize, " ");
        strcat_s(op5, opcodeSize, op6);
        strcat_s(op5, opcodeSize, " ");
        strcat_s(op5, opcodeSize, op7);
        strcat_s(op5, opcodeSize, " ");
        strcat_s(op5, opcodeSize, op8);
        strcpy_s(op6, opcodeSize, op9);
        strcpy_s(op7, opcodeSize, op10);
        strcpy_s(op8, opcodeSize, op11);
        strcpy_s(op9, opcodeSize, op12);
      ...
      {
        strcat_s(op6, opcodeSize, " ");
        strcat_s(op6, opcodeSize, op7);
        strcat_s(op6, opcodeSize, " ");
        strcat_s(op6, opcodeSize, op8);
        strcat_s(op6, opcodeSize, " ");
        strcat_s(op6, opcodeSize, op9);
        strcpy_s(op7, opcodeSize, op10);
        strcpy_s(op8, opcodeSize, op11);
        strcpy_s(op9, opcodeSize, op12);
        strcpy_s(op10, opcodeSize, op13);
      ...
      {
        strcat_s(op7, opcodeSize, " ");
        strcat_s(op7, opcodeSize, op8);
        strcat_s(op7, opcodeSize, " ");
        strcat_s(op7, opcodeSize, op9);
        strcat_s(op7, opcodeSize, " ");
        strcat_s(op7, opcodeSize, op10);
        strcpy_s(op8, opcodeSize, op11);
        strcpy_s(op9, opcodeSize, op12);
        strcpy_s(op10, opcodeSize, op13);
        strcpy_s(op11, opcodeSize, op14);
      ...
      while(!strcmp(op2, "+"))
      {
        strcat_s(op1, opcodeSize, op2);
        strcat_s(op1, opcodeSize, op3);
        strcpy_s(op2, opcodeSize, op4);
        strcpy_s(op3, opcodeSize, op5);
        strcpy_s(op4, opcodeSize, op6);
        strcpy_s(op5, opcodeSize, op7);
        strcpy_s(op6, opcodeSize, op8);
        strcpy_s(op7, opcodeSize, op9);
        strcpy_s(op8, opcodeSize, op10);
        strcpy_s(op9, opcodeSize, op11);
        strcpy_s(op10, opcodeSize, op12);
        strcpy_s(op11, opcodeSize, op13);
        strcpy_s(op12, opcodeSize, op14);
        strcpy_s(op13, opcodeSize, op15);
        op14[0] = 0;
        op15[0] = 0;
      }
      while(!strcmp(op3, "+"))
  ...
  string replaceInt(string input)
  {
    ...
    if(floor(number) != number)
      return input;
    char buffer[64];
    sprintf_s(buffer, 64, "%d", (int)number);
    return buffer;
  }
  ...
  void truncateTexturePos(char *op, const char *textype)
  {
    ...
    else
    {
      ...
      int pos = 5;
      if(!strncmp(textype, "Texture1D<", strlen("Texture1D<")))
        pos = 2;    // float .x
      else if(!strncmp(textype, "Texture2DMS<", strlen("Texture2DMS<")))
        pos = 3;    // float2 .xy
      else if(!strncmp(textype, "Texture1DArray<", strlen("Texture1DArray<")))
        pos = 3;
      else if(!strncmp(textype, "Texture2D<", strlen("Texture2D<")))
        pos = 3;
      else if(!strncmp(textype, "Texture2DMSArray<", strlen("Texture2DMSArray<")))
        pos = 3;
      else if(!strncmp(textype, "Texture2DArray<", strlen("Texture2DArray<")))
        pos = 4;    // float3 .xyz
      else if(!strncmp(textype, "Texture3D<", strlen("Texture3D<")))
        pos = 4;
      else if(!strncmp(textype, "TextureCube<", strlen("TextureCube<")))
        pos = 4;
      else if(!strncmp(textype, "TextureCubeArray<", strlen("TextureCubeArray<")))
        pos = 5;    // float4 .xyzw
      else
        logDecompileError("  unknown texture type for truncation: " + string(textype));
      cpos = strrchr(op, '.');
      cpos[pos] = 0;
    }
  }

  // This routine was expecting only r0.xyz type input parameters, but we can also get
  // float4(0,0,0,0) type inputs as constants to things like .Load().  If it's a constant of any
  // form, leave it unchanged. The reason there is a second version of this .xyzw truncator, is
  // because this version is for the _LD operands, and for that version it's one parameter larger.
  // Texture2D is 3 components, not 2 for this one. The extra bracket for comparison is to avoid
  // early mismatch like Texture1D instead of Texture1DArray.
  void truncateTextureLoadPos(char *op, const char *textype)
  {
    ...
    if(!strncmp(textype, "Buffer<", strlen("Buffer<")))
      pos = 2;    // int  .x
    else if(!strncmp(textype, "Texture1D<", strlen("Texture1D<")))
      pos = 3;    // int2 .xy
    else if(!strncmp(textype, "Texture2DMS<", strlen("Texture2DMS<")))
      pos = 3;
    else if(!strncmp(textype, "Texture1DArray<", strlen("Texture1DArray<")))
  ...
  // In the microsoft world, 10 year old C specs are not interesting, so no stdclib function exists
  // here for float, and the std::hexfloat i/o stream fails to handle 32 bits properly. It's quite
  // sad how weak this stuff is. This took quite a long time to figure out how to make it work
  // properly.  The easist answer seemed to be to use reinterpret_cast from uint to float, but that
  // appears to be unnecessary, as the %x assigned directly to a float seems to work properly (but
  // is not documented.)

  // 1/1/15: New variant of l(-1) as an int, but used after a boolean state, so it's a comparison,
  // not a bitand.
  //  This would generate a QNAN as float. Made the hex comparison more strict with 0x%x, and thus
  //  that will fail if it's an int type, and allow for converting that variant.
  ...
  void convertHexToFloat(char *target)
  {
    ...
    if(target[0] == 'l')
    {
      ...
      count = sscanf_s(target, "l(0x%x,0x%x,0x%x,0x%x)", (unsigned int *)&lit[0],
                       (unsigned int *)&lit[1], (unsigned int *)&lit[2], (unsigned int *)&lit[3]);
      if(count != 0)
      {
  ...
  char *convertToInt(char *target)
  {
    ...
    if(!strncmp(target + isMinus, "int", 3) || !strncmp(target + isMinus, "uint", 4))
      return target;
    if(!strncmp(target + isMinus, "float", 5))
    {
      ...
      buffer[0] = 0;
      if(isMinus)
        strcpy_s(buffer, opcodeSize, "-");

      if(size == 2)
        sprintf_s(buffer + strlen(buffer), opcodeSize - strlen(buffer), "int2(%d,%d)", (int)f0,
                  (int)f1);
      else if(size == 3)
        sprintf_s(buffer + strlen(buffer), opcodeSize - strlen(buffer), "int3(%d,%d,%d)", (int)f0,
                  (int)f1, (int)f2);
      else if(size == 4)
        sprintf_s(buffer + strlen(buffer), opcodeSize - strlen(buffer), "int4(%d,%d,%d,%d)",
    ...
    if(pos)
    {
      ...
      if(size == 1)
        sprintf_s(buffer, opcodeSize, "(int)%s", target);
      else
        sprintf_s(buffer, opcodeSize, "(int%zu)%s", size, target);
      strcpy_s(target, opcodeSize, buffer);
    }
    return target;
  }
  ...
  char *convertToUInt(char *target)
  {
    ...
    if(!strncmp(target + isMinus, "float", 5))
    {
      ...
      if(size == 2)
        sprintf_s(buffer + strlen(buffer), opcodeSize - strlen(buffer), "uint2(%d,%d)", (int)f0,
                  (int)f1);
      else if(size == 3)
        sprintf_s(buffer + strlen(buffer), opcodeSize - strlen(buffer), "uint3(%d,%d,%d)", (int)f0,
                  (int)f1, (int)f2);
      else if(size == 4)
        sprintf_s(buffer + strlen(buffer), opcodeSize - strlen(buffer), "uint4(%d,%d,%d,%d)",
                  (int)f0, (int)f1, (int)f2, (int)f3);
    ...
    if(pos)
    {
      size_t size = strlen(pos + 1);
      if(size == 1)
        sprintf_s(buffer, opcodeSize, "(uint)%s", target);
      else
        sprintf_s(buffer, opcodeSize, "(uint%zu)%s", size, target);
      strcpy_s(target, opcodeSize, buffer);
    }
    return target;
  }
    ...
    if(o.eType == OPERAND_TYPE_IMMEDIATE32)
    {
      ...
      if(!strncmp(op, "l(1.#INF00", strlen("l(1.#INF00")) || abs(oldValue - o.afImmediates[0]) < 0.1)
      {
        if(o.iNumComponents == 4)
          sprintf_s(op, opcodeSize, "l(%.6f, %.6f, %.6f, %.6f)", o.afImmediates[0],
                    o.afImmediates[1], o.afImmediates[2], o.afImmediates[3]);
        else if(o.iNumComponents == 3)
          sprintf_s(op, opcodeSize, "l(%.6f, %.6f, %.6f)", o.afImmediates[0], o.afImmediates[1],
                    o.afImmediates[2]);
      ...
      if(screenToWorldMatrix1 || screenToWorldMatrix2)
      {
        ...
        if(screenToWorldPos)
        {
          ...
          if(lastPos && lastPos != mOutput.data())
          {
            ...
            mOutput.insert(mOutput.begin() + (pos - mOutput.data()), viewDirectionDecl,
                           viewDirectionDecl + strlen(viewDirectionDecl));
          }
          // Add view direction calculation.
          char buf[512];
          if(screenToWorldMatrix1)
            sprintf_s(buf, 512, "  viewDirection = float3(%s);\n", BackProject_Vector1.c_str());
          else
          ...
          if(viewProjectMatrix)
          {
            ...
            char buffer[256];
            sprintf_s(buffer, 256, "%s.x -= separation * (%s.w - convergence);\n",
      ...
      if(!isMono && mFixSvPosition && mUsesProjection && !mSV_Position.empty())
      {
        ...
        if(positionValue != mOutputRegisterValues.end())
        {
              ...
              i != mOutputRegisterValues.end(); ++i)
          {
            ...
            if(dotPos > 0)
              rvalue2 = i->second.substr(0, dotPos);
            if(!rvalue2.compare(rvalue))
            {
              ...
              if(outputReg.find('.') != string::npos)
                outputReg = outputReg.substr(0, outputReg.find('.'));
              sprintf_s(buffer, 256, "\n%s.x += separation * (%s.w - convergence);",
                        outputReg.c_str(), outputReg.c_str());
    ...
    if(!mShaderType.substr(0, 3).compare("ps_"))
    {
      ...
      if(depthTexture != mTextureNames.end())
      {
        ...
        if(!ZRepair_Dependencies1.size() || found == (1 << ZRepair_Dependencies1.size()) - 1)
        {
          ...
          sprintf_s(op1, opcodeSize, " = %s.Sample", ZRepair_DepthTexture1.c_str());
          ...
          while(pos)
          {
            ...
            if(!wposAvailable)
            {
              sprintf_s(buf, 512,
                        ...
                        "float zpos = %s;\n"
                        "float wpos = 1.0 / zpos;\n",
                        depthBufferStatement.c_str(), ZRepair_DepthTextureReg1,
                        ZRepair_ZPosCalc1.c_str());
            }
            else
            {
              sprintf_s(buf, 512,
            ...
            else if(!wposAvailable)
            {
              ...
              while(*pos != '\n')
                --pos;
              // mCodeStartPos = mOutput.insert(mOutput.begin() + (pos + 1 - mOutput.data()), buf,
              // buf + strlen(buf)) - mOutput.begin();
              vector<char>::iterator iter = mOutput.insert(
                  mOutput.begin() + (pos + 1 - mOutput.data()), buf, buf + strlen(buf));
              mCodeStartPos = iter - mOutput.begin();
              mCodeStartPos += strlen(buf);
            }
            else
      ...
      if(!wposAvailable)
      {
        ...
        if(depthTexture != mTextureNames.end())
        {
          ...
          if(!ZRepair_Dependencies2.size() || found == (1 << ZRepair_Dependencies2.size()) - 1)
          {
            mOutput.push_back(0);
            // Search depth texture usage.
            sprintf_s(op1, opcodeSize, " = %s.Sample", ZRepair_DepthTexture2.c_str());
            char *pos = strstr(mOutput.data(), op1);
            if(pos)
            {
              char *bpos = pos;
              ...
              sprintf_s(buf, 256,
                        ...
                        "float zpos = %s;\n"
                        "float wpos = 1.0 / zpos;\n",
                        depthBufferStatement.c_str(), ZRepair_DepthTextureReg2,
                        ZRepair_ZPosCalc2.c_str());

              // There are a whole series of fixes to the statements that are doing mOutput.insert()
              // - mOutput.begin(); We would get a series of Asserts (vector mismatch), but only
              // rarely, usually at starting a new game in AC3. The problem appers to be that
              // mOutput.begin() was being calculated BEFORE the mOutput.insert(), which seems to be
              // the compiler optimizing. The problem seems to be that the Insert could move the
              // mOutput array altogether, and because the funny use of vector<char> the vector
              // winds up being closeset to a char* pointer.  With the pointer moved, the saved off
              // mOutput.begin() was invalid and would cause the debug Assert.
              // By calculating that AFTER the insert has finished, we can avoid this rare buffer
              // movement. I fixed every instance I could find of this insert-begin, not just the
              // one that crashed. Previous code left as example of expected behavior, and
              // documentation for where changes happened. It took two days to figure out, it's
              // worth some comments.

              if(constantDeclaration)
              {
                // Copy depth texture usage to top.
                // mCodeStartPos = mOutput.insert(mOutput.begin() + mCodeStartPos, buf, buf +
                // strlen(buf)) - mOutput.begin();
                vector<char>::iterator iter =
                    mOutput.insert(mOutput.begin() + mCodeStartPos, buf, buf + strlen(buf));
                mCodeStartPos = iter - mOutput.begin();
                mCodeStartPos += strlen(buf);
      ...
      if(!wposAvailable)
      {
        ...
        for(positionTexture = mTextureNames.begin(); positionTexture != mTextureNames.end();
            ++positionTexture)
        {
        ...
        if(positionTexture != mTextureNames.end())
        {
          ...
          sprintf_s(op1, opcodeSize, " = %s.Sample", ZRepair_PositionTexture.c_str());
          char *pos = strstr(mOutput.data(), op1);
          if(pos)
          {
            char *bpos = pos;
            ...
            sprintf_s(calcStatement, 256, ZRepair_WorldPosCalc.c_str(), buf);
            sprintf_s(buf, 512,
                      "\nfloat3 worldPos = %s;"
            ...
            // mCodeStartPos = mOutput.insert(mOutput.begin() + (pos - mOutput.data()), buf, buf +
            // strlen(buf)) - mOutput.begin();
            vector<char>::iterator iter =
      ...
      {
        const char *INJECT_HEADER =
            "float4 zpos4 = InjectedDepthTexture.Load((int3) injectedScreenPos.xyz);\n"
            "float zpos = zpos4.x - 1;\n"
            "float wpos = 1.0 / zpos;\n";
        // Copy depth texture usage to top.
        // mCodeStartPos = mOutput.insert(mOutput.begin() + mCodeStartPos, INJECT_HEADER,
        // INJECT_HEADER + strlen(INJECT_HEADER)) - mOutput.begin();
        ...
        mOutput.insert(mOutput.begin() + (pos - mOutput.data()), PARAM_HEADER,
                       PARAM_HEADER + strlen(PARAM_HEADER));
        ...
        if(keyFind != mCBufferData.end())
        {
          ...
          if(!stereoParamsWritten)
          {
            if(mOutput[mCodeStartPos] != '\n')
              --mCodeStartPos;
            // mCodeStartPos = mOutput.insert(mOutput.begin() + mCodeStartPos, StereoDecl,
            // StereoDecl + strlen(StereoDecl)) - mOutput.begin();
            vector<char>::iterator iter = mOutput.insert(
                mOutput.begin() + mCodeStartPos, StereoDecl, StereoDecl + strlen(StereoDecl));
            mCodeStartPos = iter - mOutput.begin();
            mCodeStartPos += strlen(StereoDecl);
            stereoParamsWritten = true;
          }

          for(vector<string>::iterator invT = InvTransforms.begin(); invT != InvTransforms.end();
              ++invT)
          {
            char buf[128];
            sprintf_s(buf, 128, " %s._m00", invT->c_str());
          ...
          {
            size_t offset = strstr(mOutput.data(), "void main(") -
                            mOutput.data();    // pointer difference, but only used as offset.
            while(offset < mOutput.size())
            {
                 ...
                 (pos[3] == 'x' || pos[3] == 'y' || pos[3] == 'z'))
              {
                ...
                strcpy_s(op2, opcodeSize, op1);
                applySwizzle(".x", op2);
                strcpy_s(op3, opcodeSize, op1);
                applySwizzle(".y", op3);
                strcpy_s(op4, opcodeSize, op1);
                applySwizzle(".z", op4);
                char buf[512];
                ...
                sprintf_s(buf, 512,
                          ...
                          "\n%s += viewDirection.%c * separation * (wpos - convergence) * "
                          "stereoPos%dMul.x;"
                          "\n%s += viewDirection.%c * separation * (wpos - convergence) * "
                          "stereoPos%dMul.y;"
                          "\n%s += viewDirection.%c * separation * (wpos - convergence) * "
                          "stereoPos%dMul.z;",
                          uuidVar, ObjectPos_MUL1.c_str(), op2, lightPosDecl[0], uuidVar, op3,
                          lightPosDecl[1], uuidVar, op4, lightPosDecl[2], uuidVar);
                ++uuidVar;
                pos = strchr(pos, '\n');
                mOutput.insert(mOutput.begin() + (pos - mOutput.data()), buf, buf + strlen(buf));
                offset += strlen(buf);
            ...
            while(offset < mOutput.size())
            {
                 ...
                 (pos[3] == 'x' || pos[3] == 'y' || pos[3] == 'z'))
              {
                ...
                sprintf_s(buf, 512,
                          ...
                          "\n%s += viewDirection.%c * separation * (wpos - convergence) * "
                          "stereoPos%dMul.z;",
                          uuidVar, ObjectPos_MUL2.c_str(), op2, spotPosDecl[0], uuidVar, op3,
                          spotPosDecl[1], uuidVar, op4, spotPosDecl[2], uuidVar);
                ++uuidVar;
                pos = strchr(pos, '\n');
                mOutput.insert(mOutput.begin() + (pos - mOutput.data()), buf, buf + strlen(buf));
                offset += strlen(buf);
          ...
          if(!MatrixPos_ID1.empty())
          {
            ...
            if(pos1 && pos2 && pos3)
            {
              string regName1(pos1 + ShadowPos1.length(),
                              strchr(pos1 + ShadowPos1.length(), '.') + 2);
              string regName2(pos2 + ShadowPos2.length(),
                              strchr(pos2 + ShadowPos2.length(), '.') + 2);
              string regName3(pos3 + ShadowPos3.length(),
                              strchr(pos3 + ShadowPos3.length(), '.') + 2);
              ...
              sprintf_s(
                  ...
                  uuidVar, MatrixPos_MUL1.c_str(), regName1.c_str(), uuidVar, regName2.c_str(),
                  uuidVar, regName3.c_str(), uuidVar);
              ++uuidVar;
              mOutput.insert(mOutput.begin() + (pos - mOutput.data()), buf, buf + strlen(buf));
              mPatched = true;
  ...
  string GetComponentStrFromInstruction(Instruction *instr, int opIndex)
  {
    ...
    else if(instr->asOperands[opIndex].eSelMode == OPERAND_4_COMPONENT_SWIZZLE_MODE)
    {
      for(int i = 0; i < 4; i++)
      {
        sprintf_s(buff, opcodeSize, "%s%s", buff,
                  component[instr->asOperands[opIndex].aui32Swizzle[i]]);
      }
    }
  ...
  int IsInstructionOperandSame(Instruction *instr1, int opIndex1, Instruction *instr2, int opIndex2,
                               const char *instr1Op1 = NULL, const char *instr2Op1 = NULL)
  {
    Operand &op1r = instr1->asOperands[opIndex1];
    Operand &op2r = instr2->asOperands[opIndex2];
    ...
    sprintf_s(buff1, opcodeSize, "r%d.%s", op1r.ui32RegisterNumber, component1.c_str());
    sprintf_s(buff2, opcodeSize, "r%d.%s", op2r.ui32RegisterNumber, component2.c_str());
    ...
    if(same)
    {
      if(((op1r.eModifier == OPERAND_MODIFIER_NEG || op1r.eModifier == OPERAND_MODIFIER_ABSNEG) &&
          (op2r.eModifier == OPERAND_MODIFIER_NONE || op2r.eModifier == OPERAND_MODIFIER_ABS)) ||
         ((op2r.eModifier == OPERAND_MODIFIER_NEG || op2r.eModifier == OPERAND_MODIFIER_ABSNEG) &&
          (op1r.eModifier == OPERAND_MODIFIER_NONE || op1r.eModifier == OPERAND_MODIFIER_ABS)))
      {
        return 2;
      }
  ...
  void ParseCode(Shader *shader, const char *c, size_t size)
  {
    ...
    while(pos < size && iNr < inst->size())
    {
      ...
      if(!strncmp(c + pos, "#line", 5) || !strncmp(c + pos, "undecipherable", 14))
      {
        while(c[pos] != 0x0a && pos < size)
          pos++;
        pos++;
      ...
      // statement, 	op1, op2, op3, op4, op5, op6, op7, op8, op9, op10, op11, op12, op13, op14, op15);
      //

      if(!strncmp(statement, "vs_", 3) || !strncmp(statement, "ps_", 3) ||
      ...
      {
        sprintf_s(buffer, 512, "  const float4 icb[] =");
        mOutput.insert(mOutput.end(), buffer, buffer + strlen(buffer));
        ...
        if(op2[0] == 't')
        {
          ...
          if(i == mTextureNames.end())
          {
            ...
            //mCodeStartPos += strlen(buffer);
                  ...
                  statement, "dcl_resource_texture2darray"))    // dcl_resource_texture2darray
                                                                // (float,float,float,float) t0
      {
        if(op2[0] == 't')
        {
          int bufIndex = 0;
      ...
      else if(!strncmp(statement, "dcl_resource_texture2dms",
                       strlen("dcl_resource_texture2dms")))    // dcl_resource_texture2dms(8)
                                                               // (float,float,float,float) t4
      {
        if(op2[0] == 't')
        {
          int bufIndex = 0;
          ...
          {
            logDecompileError("Error parsing dcl_resource_texture2dms array dimension: " +
                              string(statement));
            return;
          }
          ...
          if(i == mTextureNames.end())
          {
            ...
            if(dim == 0)
              sprintf_s(buffer, 512, "Texture2DMS<float4> t%d : register(t%d);\n\n", bufIndex,
                        bufIndex);
            else
      ...
      else if(!strcmp(statement, "dcl_indexrange"))
      {
        ...
        sprintf_s(buffer, 512, "  float4 v[%d] = { ", numIndex);
      ...
      else if(!strcmp(statement, "dcl_indexableTemp"))
      {
        ...
        sprintf_s(buffer, 512, "  float4 %s[%d];\n", varName, numIndex);
        mOutput.insert(mOutput.end(), buffer, buffer + strlen(buffer));
      }
      else if(!strcmp(statement, "dcl_temps"))
      {
        ...
        int numTemps;
        sscanf_s(c + pos, "%s %d", statement, 128, &numTemps);
        for(int i = 0; i < numTemps; ++i)
        {
          sprintf_s(buffer, 512, "r%d,", i);
          mOutput.insert(mOutput.end(), buffer, buffer + strlen(buffer));
        }
        mOutput.pop_back();
        mOutput.push_back(';');
      ...
      else
      {
        switch(instr->eOpcode)
        {
          ...
          case OPCODE_MOV:
            ...
            if(!instr->bSaturate)
              sprintf_s(buffer, 512, "  %s = %s;\n", writeTarget(op1), ci(op2).c_str());
            else
              sprintf_s(buffer, 512, "  %s = saturate(%s);\n", writeTarget(op1), ci(op2).c_str());
            appendOutput(buffer);
            if(op1[0] == 'o')
            {
              char *dotPos = strchr(op1, '.');
              if(dotPos)
                *dotPos = 0;
          ...
          case OPCODE_RCP:
            ...
            if(!instr->bSaturate)
              sprintf_s(buffer, 512, "  %s = rcp(%s);\n", writeTarget(op1), ci(op2).c_str());
            else
              sprintf_s(buffer, 512, "  %s = saturate(rcp(%s));\n", writeTarget(op1),
                        ci(op2).c_str());
            appendOutput(buffer);
            break;
          ...
          case OPCODE_F32TOF16:
            remapTarget(op1);
            applySwizzle(op1, op2);
            sprintf_s(buffer, 512, "  %s = f32tof16(%s);\n", writeTarget(op1), ci(op2).c_str());
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          ...
          case OPCODE_F16TOF32:
            remapTarget(op1);
            applySwizzle(op1, op2);
            sprintf_s(buffer, 512, "  %s = f16tof32(%s);\n", writeTarget(op1), ci(op2).c_str());
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          ...
          case OPCODE_FRC:
            ...
            applySwizzle(op1, op2);
            if(!instr->bSaturate)
              sprintf_s(buffer, 512, "  %s = frac(%s);\n", writeTarget(op1), ci(op2).c_str());
            else
              sprintf_s(buffer, 512, "  %s = saturate(frac(%s));\n", writeTarget(op1),
                        ci(op2).c_str());
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          ...
          case OPCODE_MUL:
            ...
            mMulTarget = op1;
            if(!instr->bSaturate)
              sprintf_s(buffer, 512, "  %s = %s * %s;\n", writeTarget(op1), ci(op3).c_str(),
                        ci(op2).c_str());
            else
              sprintf_s(buffer, 512, "  %s = saturate(%s * %s);\n", writeTarget(op1),
                        ci(op3).c_str(), ci(op2).c_str());
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          ...
          case OPCODE_IMUL:
            ...
            sprintf_s(buffer, 512, "  %s = %s * %s;\n", writeTarget(op2),
                      ci(convertToInt(op3)).c_str(), ci(convertToInt(op4)).c_str());
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          ...
          case OPCODE_DIV:
            ...
            // UDIV instruction also could damage the original register before finishing, giving
            // wrong results. e.g. udiv r0.x, r1.x, r0.x, r0.y variant: udiv null, r1.xy, r3.zwzz,
            // r2.zwzz To fix this, we are using the temp variables declared at the top. expected
            // output:
            //   uiDest.x = (uint)r0.x / (uint)r0.y;
            //   r1.x = (uint)r0.x % (uint)r0.y;
            //   r0.x = uiDest.x;
            //
            // This will swizzle based on either op1 or op2, as long as it's not null.  It's not
            // clear whether the swizzle is allowed to vary for each half of the instruction, like
            // xy for /, zw for %. To allow for that, we'll set the temp registers with full
            // swizzle, then only use the specific parts required for each half, as the safest
            // approach.  Might not generate udiv though. Also removed saturate code, because udiv
            // does not specify that. Creates operand copies to applySwizzle to unchanged operands,
            // as constant l values are otherwise damaged.
          case OPCODE_UDIV:
          {
            remapTarget(op1);
            remapTarget(op2);
          ...
          case OPCODE_IADD:
            ...
            {
              if(op2[0] == '-')
                strcpy_s(op2, opcodeSize, op2 + 1);
              else if(op3[0] == '-')
                strcpy_s(op3, opcodeSize, op3 + 1);
              sprintf_s(buffer, 512, "  %s = (%s ? -1 : 0) + (%s ? 1 : 0);\n", writeTarget(op1),
                        ci(convertToInt(op2)).c_str(), ci(convertToInt(op3)).c_str());
            }
            else
              sprintf_s(buffer, 512, "  %s = %s + %s;\n", writeTarget(op1),
                        ci(convertToInt(op2)).c_str(), ci(convertToInt(op3)).c_str());
            ...
            // AND opcodes were generating bad code, as the hex constants were being converted
            // badly. The most common case was 0x3f800000 being converted directly to integer
            // decimal of 1065353216, instead of the most likely answer of floating point 1.0f. This
            // also happened for conversion of Pi. There are bitmasks used for AND, and those need
            // to stay as Hex constants. But anything used after IF statements/booleans, needs to be
            // converted as float. Rather than modify applySwizzle for this single opcode, it makes
            // more sense to convert them here, if they are to be used in boolean operations.  We
            // make a copy of the incoming operands, so that we can applySwizzle in order to be able
            // to look up isBoolean properly.  Can be r3.xxxy, and becomes r3.xy. That applySwizzle
            // damages constants though, so if we are boolean, we'll use the original l() value.
          case OPCODE_AND:
            remapTarget(op1);
            strcpy_s(op12, opcodeSize, op2);
            strcpy_s(op13, opcodeSize, op3);
            applySwizzle(op1, op2);
            ...
            if(isBoolean(op2) || isBoolean(op3))
            {
              ...
              sprintf_s(buffer, 512, "  %s = %s ? %s : 0;\n", writeTarget(op1), ci(cmp).c_str(),
                        ci(arg).c_str());
              appendOutput(buffer);
            }
            else
          ...
          case OPCODE_XOR:
            ...
            // Curiously enough, the documentation for ISHR and ISHL is wrong, and documents the
            // parameters backwards.
            // http://msdn.microsoft.com/en-us/library/windows/desktop/hh447145(v=vs.85).aspx
            // This was proven by looking at actual game ASM, and trying to make the HLSL match the
            // generated ASM. It is the C standard of: shift-expression << additive-expression So,
            // we need op2 as the Shift-Expression, op3 as the # of bits to shift.
          case OPCODE_ISHR:
            remapTarget(op1);
            applySwizzle(op1, op2, true);
            applySwizzle(op1, op3, true);
            sprintf_s(buffer, 512, "  %s = %s >> %s;\n", writeTarget(op1),
                      ci(convertToUInt(op2)).c_str(), ci(convertToInt(op3)).c_str());
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          ...
          case OPCODE_USHR:
            ...
            applySwizzle(op1, op3, true);
            sprintf_s(buffer, 512, "  %s = %s >> %s;\n", writeTarget(op1),
                      ci(convertToUInt(op2)).c_str(), ci(convertToUInt(op3)).c_str());
            appendOutput(buffer);
            removeBoolean(op1);
            break;

            // Code generation for this weird instruction is tuned to indent the way we want,
            // and still look like a single instruction.  Still has weird indent in middle of
            // instruction, but it seems more valuable to have it be a single line.
          case OPCODE_UBFE:
          {
            remapTarget(op1);
            removeBoolean(op1);
            ...
            int idx = 0;
            char *pop1 = strrchr(op1, '.');
            *pop1 = 0;
            while(*++pop1)
            {
              sprintf_s(op5, opcodeSize, "%s.%c", op1, *pop1);
              sprintf_s(buffer, 512, "  if (%s == 0) %s = 0; else if (%s+%s < 32) { ",
                        ci(GetSuffix(op2, idx)).c_str(), writeTarget(op5),
                        ci(GetSuffix(op2, idx)).c_str(), ci(GetSuffix(op3, idx)).c_str());
              appendOutput(buffer);
              sprintf_s(buffer, 512, "%s = (int)%s << (32-(%s + %s)); %s = (uint)%s >> (32-%s); ",
                        writeTarget(op5), ci(GetSuffix(op4, idx)).c_str(),
                        ci(GetSuffix(op2, idx)).c_str(), ci(GetSuffix(op3, idx)).c_str(),
                        writeTarget(op5), writeTarget(op5), ci(GetSuffix(op2, idx)).c_str());
              appendOutput(buffer);
              sprintf_s(buffer, 512, " } else %s = (uint)%s >> %s;\n", writeTarget(op5),
                        ci(GetSuffix(op4, idx)).c_str(), ci(GetSuffix(op3, idx)).c_str());
              appendOutput(buffer);
              ++idx;
            }
            break;
          ...
          case OPCODE_EXP:
            ...
            applySwizzle(op1, op2);
            if(!instr->bSaturate)
              sprintf_s(buffer, 512, "  %s = exp2(%s);\n", writeTarget(op1), ci(op2).c_str());
            else
              sprintf_s(buffer, 512, "  %s = saturate(exp2(%s));\n", writeTarget(op1),
                        ci(op2).c_str());
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          ...
          case OPCODE_LOG:
            if(shader->dx9Shader)
            {
              ...
              if(nextIns[0]->eOpcode == OPCODE_LOG && nextIns[1]->eOpcode == OPCODE_LOG &&
                 nextIns[2]->eOpcode == OPCODE_MUL && nextIns[3]->eOpcode == OPCODE_EXP &&
                 nextIns[4]->eOpcode == OPCODE_EXP && nextIns[5]->eOpcode == OPCODE_EXP &&
                 instr->asOperands[1].ui32RegisterNumber ==
                     nextIns[0]->asOperands[1].ui32RegisterNumber &&
                 nextIns[0]->asOperands[1].ui32RegisterNumber ==
                     nextIns[1]->asOperands[1].ui32RegisterNumber)
              {
                string op1Str;
                string op3Str;
                ...
                sprintf_s(buffer, 512, "  r%d.%s%s%s = pow(r%d.%s%s%s, %s);\n",
                          nextIns[3]->asOperands[0].ui32RegisterNumber,
                          GetComponentStrFromInstruction(nextIns[3], 0).c_str(),
                          GetComponentStrFromInstruction(nextIns[4], 0).c_str(),
                          GetComponentStrFromInstruction(nextIns[5], 0).c_str(),
                          instr->asOperands[1].ui32RegisterNumber,
                          GetComponentStrFromInstruction(instr, 1).c_str(),
                          GetComponentStrFromInstruction(nextIns[0], 1).c_str(),
              ...
              else
              {
                ...
                applySwizzle(op1, op2);
                if(!instr->bSaturate)
                  sprintf_s(buffer, 512, "  %s = log2(%s);\n", writeTarget(op1), ci(op2).c_str());
                else
                  sprintf_s(buffer, 512, "  %s = saturate(log2(%s));\n", writeTarget(op1),
                            ci(op2).c_str());
                appendOutput(buffer);
                removeBoolean(op1);
              }
            ...
            // This fix removes the dependency on 'statement', and codes the generated HLSL line
            // directly. We are guessing, but it appears that this was a left-over from a conversion
            // to using the James-Jones opcode parser as the primary parser.
          case OPCODE_SQRT:
            remapTarget(op1);
            applySwizzle(op1, op2);
            if(!instr->bSaturate)
              sprintf_s(buffer, 512, "  %s = sqrt(%s);\n", writeTarget(op1), ci(op2).c_str());
            else
              sprintf_s(buffer, 512, "  %s = saturate(sqrt(%s));\n", writeTarget(op1),
                        ci(op2).c_str());
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          ...
          case OPCODE_MIN:
            ...
            applySwizzle(op1, fixImm(op3, instr->asOperands[2]));
            if(!instr->bSaturate)
              sprintf_s(buffer, 512, "  %s = min(%s, %s);\n", writeTarget(op1), ci(op2).c_str(),
                        ci(op3).c_str());
            else
              sprintf_s(buffer, 512, "  %s = saturate(min(%s, %s));\n", writeTarget(op1),
                        ci(op2).c_str(), ci(op3).c_str());
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          ...
          case OPCODE_UMIN:
            ...
            applySwizzle(op1, fixImm(op2, instr->asOperands[1]));
            applySwizzle(op1, fixImm(op3, instr->asOperands[2]));
            sprintf_s(buffer, 512, "  %s = min(asuint(%s), asuint(%s));\n", writeTarget(op1),
                      ci(op2).c_str(), ci(op3).c_str());
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          ...
          case OPCODE_MAX:
            ...
            applySwizzle(op1, fixImm(op3, instr->asOperands[2]));
            if(!instr->bSaturate)
              sprintf_s(buffer, 512, "  %s = max(%s, %s);\n", writeTarget(op1), ci(op2).c_str(),
                        ci(op3).c_str());
            else
              sprintf_s(buffer, 512, "  %s = saturate(max(%s, %s));\n", writeTarget(op1),
                        ci(op2).c_str(), ci(op3).c_str());
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          ...
          case OPCODE_MAD:
            ...
            if(shader->dx9Shader)
            {
              ...
              if(nextIns->eOpcode == OPCODE_MAD &&
                 IsInstructionOperandSame(instr, 3, nextIns, 3,
                                          GetComponentStrFromInstruction(instr, 0).c_str(),
                                          GetComponentStrFromInstruction(nextIns, 0).c_str()) == 2 &&
                 IsInstructionOperandSame(instr, 0, nextIns, 2, NULL,
                                          GetComponentStrFromInstruction(nextIns, 0).c_str()) == 1)
              {
                applySwizzle(op1, op2);
                applySwizzle(op1, op3);
            ...
            /*
            if (mLastStatement && mLastStatement->eOpcode == OPCODE_MUL && strstr(op4,
            mMulTarget.c_str()) && mMulTarget.compare(0, 3, mMulOperand, mMulOperand[0] == '-' ? 1 :
            0, 3) && mMulTarget.compare(0, 3, mMulOperand2, mMulOperand2[0] == '-' ? 1 : 0, 3)) sprintf(op4
            + ((op4[0] == '-') ? 1 : 0), "(%s * %s)", mMulOperand.c_str(), mMulOperand2.c_str());
            */
            if(!instr->bSaturate)
              sprintf_s(buffer, 512, "  %s = %s * %s + %s;\n", writeTarget(op1), ci(op2).c_str(),
                        ci(op3).c_str(), ci(op4).c_str());
            else
              sprintf_s(buffer, 512, "  %s = saturate(%s * %s + %s);\n", writeTarget(op1),
                        ci(op2).c_str(), ci(op3).c_str(), ci(op4).c_str());
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          ...
          case OPCODE_IMAD:
            ...
            sprintf_s(buffer, 512, "  %s = mad(%s, %s, %s);\n", writeTarget(op1),
                      ci(convertToInt(op2)).c_str(), ci(convertToInt(op3)).c_str(),
                      ci(convertToInt(op4)).c_str());
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          case OPCODE_UMAD:
          ...
          case OPCODE_DP2:
            ...
            if(!instr->bSaturate)
              sprintf_s(buffer, 512, "  %s = dot(%s, %s);\n", writeTarget(op1), ci(op2).c_str(),
                        ci(op3).c_str());
            else
              sprintf_s(buffer, 512, "  %s = saturate(dot(%s, %s));\n", writeTarget(op1),
                        ci(op2).c_str(), ci(op3).c_str());
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          ...
          case OPCODE_DP3:
          {
            if(shader->dx9Shader)
            {
              ...
              if(nextIns[0]->eOpcode == OPCODE_ADD && nextIns[1]->eOpcode == OPCODE_MAD &&
                 IsInstructionOperandSame(instr, 0, nextIns[0], 1) == 1 &&
                 IsInstructionOperandSame(instr, 0, nextIns[0], 2) == 1 &&
                 IsInstructionOperandSame(nextIns[0], 0, nextIns[1], 2) == 2 &&
                 IsInstructionOperandSame(instr, 1, nextIns[1], 3, NULL, outputOp1.c_str()) == 1 &&
                 IsInstructionOperandSame(instr, 2, nextIns[1], 1, NULL, outputOp1.c_str()) == 1)
              {
                // read next instruction
                for(int i = 0; i < 2; i++)
                {
                ...
                remapTarget(op1);
                sprintf_s(op2, opcodeSize, "r%d.%s", nextIns[1]->asOperands[3].ui32RegisterNumber,
                          GetComponentStrFromInstruction(nextIns[1], 3).c_str());
                sprintf_s(op3, opcodeSize, "r%d.%s", nextIns[1]->asOperands[1].ui32RegisterNumber,
                          GetComponentStrFromInstruction(nextIns[1], 1).c_str());
                applySwizzle(op1, op2);
                applySwizzle(op1, op3);

                sprintf_s(buffer, 512, "  %s = reflect(%s, %s);\n", writeTarget(op1), op2, op3);
          ...
          case OPCODE_DP4:
            if(shader->dx9Shader)
            {
              ...
              {
                applySwizzle(op1, op2);
                sprintf_s(buffer, 512, "  %s = normalize(%s);\n", writeTarget(op1), ci(op2).c_str());
                appendOutput(buffer);
          ...
          case OPCODE_DP2ADD:
            ...
            applySwizzle(".xy", op3);
            applySwizzle(".xy", op4);
            sprintf_s(buffer, 512, "  %s = dot2(%s, %s) + %s;\n", writeTarget(op1), ci(op2).c_str(),
                      ci(op3).c_str(), ci(op4).c_str());
            appendOutput(buffer);
            break;
          ...
          case OPCODE_LRP:
            ...
            applySwizzle(op1, op3);
            applySwizzle(op1, op4);
            sprintf_s(buffer, 512, "  %s = lerp(%s, %s, %s);\n", writeTarget(op1), ci(op4).c_str(),
                      ci(op3).c_str(), ci(op2).c_str());
            appendOutput(buffer);
            break;
          ...
          case OPCODE_POW:
            ...
            applySwizzle(op1, op2);
            applySwizzle(op1, op3);
            sprintf_s(buffer, 512, "  %s = pow(%s, %s);\n", writeTarget(op1), ci(op2).c_str(),
                      ci(op3).c_str());
            appendOutput(buffer);
            break;
          case OPCODE_RSQ:
          {
            ...
            applySwizzle(op1, op2);
            if(!instr->bSaturate)
              sprintf_s(buffer, 512, "  %s = 1.0 / sqrt(%s);\n", writeTarget(op1), ci(op2).c_str());
            else
              sprintf_s(buffer, 512, "  %s = saturate(rsqrt(%s));\n", writeTarget(op1),
                        ci(op2).c_str());
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          ...
          case OPCODE_ROUND_NI:
          {
            ...
            applySwizzle(op1, op2);
            if(!instr->bSaturate)
              sprintf_s(buffer, 512, "  %s = floor(%s);\n", writeTarget(op1), ci(op2).c_str());
            else
              sprintf_s(buffer, 512, "  %s = saturate(floor(%s));\n", writeTarget(op1),
                        ci(op2).c_str());
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          ...
          case OPCODE_ROUND_PI:
          {
            ...
            applySwizzle(op1, op2);
            if(!instr->bSaturate)
              sprintf_s(buffer, 512, "  %s = ceil(%s);\n", writeTarget(op1), ci(op2).c_str());
            else
              sprintf_s(buffer, 512, "  %s = saturate(ceil(%s));\n", writeTarget(op1),
                        ci(op2).c_str());
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          ...
          case OPCODE_ROUND_Z:
          {
            ...
            applySwizzle(op1, op2);
            if(!instr->bSaturate)
              sprintf_s(buffer, 512, "  %s = trunc(%s);\n", writeTarget(op1), ci(op2).c_str());
            else
              sprintf_s(buffer, 512, "  %s = saturate(trunc(%s));\n", writeTarget(op1),
                        ci(op2).c_str());
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          ...
          case OPCODE_ROUND_NE:
          {
            ...
            applySwizzle(op1, op2);
            if(!instr->bSaturate)
              sprintf_s(buffer, 512, "  %s = round(%s);\n", writeTarget(op1), ci(op2).c_str());
            else
              sprintf_s(buffer, 512, "  %s = saturate(round(%s));\n", writeTarget(op1),
                        ci(op2).c_str());
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          ...
          case OPCODE_SINCOS:
            ...
            if(!strncmp(op1, "null", 4))
              ...
              applySwizzle(op1, op3);
            if(!strncmp(op1, "null", 4))
              sprintf_s(buffer, 512, "  %s = cos(%s);\n", writeTarget(op2), ci(op3).c_str());
            else if(!strncmp(op2, "null", 4))
              sprintf_s(buffer, 512, "  %s = sin(%s);\n", writeTarget(op1), ci(op3).c_str());
            else
              sprintf_s(buffer, 512, "  sincos(%s, %s, %s);\n", ci(op3).c_str(), writeTarget(op1),
                        writeTarget(op2));
            appendOutput(buffer);
            removeBoolean(op1);
            removeBoolean(op2);
          ...
          case OPCODE_MOVC:
          {
            ...
            // if (!instr->bSaturate)
            //	sprintf_s(buffer, 512, "  %s = %s ? %s : %s;\n", writeTarget(op5), ci(op6).c_str(),
            //ci(GetSuffix(op3, idx)).c_str(), ci(GetSuffix(op4, idx)).c_str()); else 	sprintf_s(buffer,
            //512, "  %s = saturate(%s ? %s : %s);\n", writeTarget(op5), ci(op6).c_str(), ci(GetSuffix(op3,
            //idx)).c_str(), ci(GetSuffix(op4, idx)).c_str()); appendOutput(buffer);
            //	++idx;
            //}
            removeBoolean(op1);
            break;
          ...
          // Big change to all these boolean test opcodes.  All were unrolled and generated a code
          // line per component. To make the boolean tests work correctly for AND, these were rolled
          // back into one, and added to the boolean set list as a complete operand, like 'r0.xyw'.
          case OPCODE_NE:
          {
            remapTarget(op1);
            applySwizzle(op1, op2);
          ...
          case OPCODE_INE:
          {
            ...
            applySwizzle(op1, op2);
            applySwizzle(op1, op3);
            sprintf_s(buffer, 512, "  %s = (int)%s != %s;\n", writeTarget(op1), ci(op2).c_str(),
                      ci(convertToInt(op3)).c_str());
            appendOutput(buffer);
            addBoolean(op1);
            break;
          }
          ...
          case OPCODE_ILT:
          {
            ...
            applySwizzle(op1, op2, true);
            applySwizzle(op1, op3, true);
            sprintf_s(buffer, 512, "  %s = (int)%s < (int)%s;\n", writeTarget(op1), ci(op2).c_str(),
                      ci(op3).c_str());
            appendOutput(buffer);
            addBoolean(op1);
            break;
          ...
          case OPCODE_ULT:
          {
            ...
            applySwizzle(op1, op2, true);
            applySwizzle(op1, op3, true);
            sprintf_s(buffer, 512, "  %s = (uint)%s < (uint)%s;\n", writeTarget(op1),
                      ci(op2).c_str(), ci(op3).c_str());
            appendOutput(buffer);
            addBoolean(op1);
            break;
          ...
          case OPCODE_SWITCH:
            sprintf_s(buffer, 512, "  switch (%s) {\n", ci(op1).c_str());
            appendOutput(buffer);
            break;
          case OPCODE_CASE:
            sprintf_s(buffer, 512, "  case %s :", ci(op1).substr(2, 1).c_str());
            appendOutput(buffer);
            break;
          ...
          case OPCODE_IF:
            ...
            if(instr->eBooleanTestType == INSTRUCTION_TEST_ZERO)
              sprintf_s(buffer, 512, "  if (%s == 0) {\n", ci(op1).c_str());
            else
              sprintf_s(buffer, 512, "  if (%s != 0) {\n", ci(op1).c_str());
            appendOutput(buffer);
            break;
          case OPCODE_ELSE:
            sprintf_s(buffer, 512, "  } else {\n");
            appendOutput(buffer);
            break;
          case OPCODE_ENDIF:
            sprintf_s(buffer, 512, "  }\n");
          ...
          case OPCODE_LOOP:
            sprintf_s(buffer, 512, "  while (true) {\n");
            appendOutput(buffer);
            break;
          case OPCODE_BREAK:
            sprintf_s(buffer, 512, "  break;\n");
            appendOutput(buffer);
            break;
          case OPCODE_BREAKC:
            applySwizzle(".x", op1);
            if(instr->eBooleanTestType == INSTRUCTION_TEST_ZERO)
              sprintf_s(buffer, 512, "  if (%s == 0) break;\n", ci(op1).c_str());
            else
              sprintf_s(buffer, 512, "  if (%s != 0) break;\n", ci(op1).c_str());
            appendOutput(buffer);
            break;
          case OPCODE_CONTINUE:
            sprintf_s(buffer, 512, "  continue;\n");
            appendOutput(buffer);
            break;
          case OPCODE_CONTINUEC:
            applySwizzle(".x", op1);
            if(instr->eBooleanTestType == INSTRUCTION_TEST_ZERO)
              sprintf_s(buffer, 512, "  if (%s == 0) continue;\n", ci(op1).c_str());
            else
              sprintf_s(buffer, 512, "  if (%s != 0) continue;\n", ci(op1).c_str());
            appendOutput(buffer);
            break;
          ...
          case OPCODE_SWAPC:
          {
            ...
            char *pop1 = strrchr(op1, '.');
            *pop1 = 0;
            char *pop2 = strrchr(op2, '.');
            if(pop2)
              *pop2 = 0;
            char *pop3 = strrchr(op3, '.');
            if(pop3)
              *pop3 = 0;
            while(*++pop1)
            {
              sprintf_s(op6, opcodeSize, "%s.%c", op1, *pop1);
              if(pop2)
                sprintf_s(op7, opcodeSize, "%s.%c", op2, *++pop2);
              else
                sprintf_s(op7, opcodeSize, "%s", op2);

              if(pop3)
                sprintf_s(op8, opcodeSize, "%s.%c", op3, *++pop3);
              else
                sprintf_s(op8, opcodeSize, "%s", op3);

              sprintf_s(buffer, 512, "  %s = (int)%s ? %s : %s; %s = (int)%s ? %s : %s;\n",
                        writeTarget(op6), ci(op8).c_str(), ci(GetSuffix(op5, idx)).c_str(),
                        ci(GetSuffix(op4, idx)).c_str(), writeTarget(op7), ci(op8).c_str(),
                        ci(GetSuffix(op4, idx)).c_str(), ci(GetSuffix(op5, idx)).c_str());
              appendOutput(buffer);
              ++idx;
            }
            break;
          ...
          case OPCODE_BFI:
          {
            ...
            *pop1 = 0;
            while(*++pop1)
            {
              ...
              sprintf_s(buffer, 512,
                        "  bitmask.%c = ((~(-1 << %s)) << %s) & 0xffffffff;\n"
                        "  %s = (((uint)%s << %s) & bitmask.%c) | ((uint)%s & ~bitmask.%c);\n",
                        *pop1, ci(GetSuffix(op2, idx)).c_str(), ci(GetSuffix(op3, idx)).c_str(),
                        writeTarget(op6), ci(GetSuffix(op4, idx)).c_str(),
                        ci(GetSuffix(op3, idx)).c_str(), *pop1, ci(GetSuffix(op5, idx)).c_str(),
                        *pop1);
              appendOutput(buffer);
              ++idx;
            }
          ...
          case OPCODE_SAMPLE:
          {
            ...
            if(shader->dx9Shader)
            {
              ...
              sprintf_s(buffer, 512, "  %s = %s.Sample(%s);\n", writeTarget(op1),
            ...
            else
            {
              ...
              if(!instr->bAddressOffset)
                sprintf_s(buffer, 512, "  %s = %s.Sample(%s, %s)%s;\n", writeTarget(op1),
                          mTextureNames[textureId].c_str(), mSamplerNames[samplerId].c_str(),
                          ci(op2).c_str(), strrchr(op3, '.'));
              else
              {
                int offsetx = 0, offsety = 0, offsetz = 0;
                sscanf_s(statement, "sample_aoffimmi(%d,%d,%d", &offsetx, &offsety, &offsetz);
                sprintf_s(buffer, 512, "  %s = %s.Sample(%s, %s, int2(%d, %d))%s;\n",
                          writeTarget(op1), mTextureNames[textureId].c_str(),
                          mSamplerNames[samplerId].c_str(), ci(op2).c_str(), offsetx, offsety,
                          strrchr(op3, '.'));
              }
              appendOutput(buffer);
              removeBoolean(op1);
          ...
          case OPCODE_SAMPLE_B:
          {
            ...
            truncateTexturePos(op2, mTextureType[textureId].c_str());
            if(!instr->bAddressOffset)
              sprintf_s(buffer, 512, "  %s = %s.SampleBias(%s, %s, %s)%s;\n", writeTarget(op1),
                        mTextureNames[textureId].c_str(), mSamplerNames[samplerId].c_str(),
                        ci(op2).c_str(), ci(op5).c_str(), strrchr(op3, '.'));
            else
            {
              int offsetx = 0, offsety = 0, offsetz = 0;
              sscanf_s(statement, "sample_b_aoffimmi_indexable(%d,%d,%d", &offsetx, &offsety,
                       &offsetz);
              sprintf_s(buffer, 512, "  %s = %s.SampleBias(%s, %s, %s, int2(%d, %d))%s;\n",
                        writeTarget(op1), mTextureNames[textureId].c_str(),
                        mSamplerNames[samplerId].c_str(), ci(op2).c_str(), ci(op5).c_str(), offsetx,
                        offsety, strrchr(op3, '.'));
            }
            appendOutput(buffer);
            removeBoolean(op1);
          ...
          case OPCODE_SAMPLE_L:
          {
            ...
            truncateTexturePos(op2, mTextureType[textureId].c_str());
            if(!instr->bAddressOffset)
              sprintf_s(buffer, 512, "  %s = %s.SampleLevel(%s, %s, %s)%s;\n", writeTarget(op1),
                        mTextureNames[textureId].c_str(), mSamplerNames[samplerId].c_str(),
                        ci(op2).c_str(), ci(op5).c_str(), strrchr(op3, '.'));
            else
            {
              int offsetx = 0, offsety = 0, offsetz = 0;
              sscanf_s(statement, "sample_l_aoffimmi_indexable(%d,%d,%d", &offsetx, &offsety,
                       &offsetz);
              sprintf_s(buffer, 512, "  %s = %s.SampleLevel(%s, %s, %s, int2(%d, %d))%s;\n",
                        writeTarget(op1), mTextureNames[textureId].c_str(),
                        mSamplerNames[samplerId].c_str(), ci(op2).c_str(), ci(op5).c_str(), offsetx,
                        offsety, strrchr(op3, '.'));
            }
          ...
          case OPCODE_SAMPLE_D:
          {
            ...
            truncateTexturePos(op2, mTextureType[textureId].c_str());
            if(!instr->bAddressOffset)
              sprintf_s(buffer, 512, "  %s = %s.SampleGrad(%s, %s, %s, %s)%s;\n", writeTarget(op1),
                        mTextureNames[textureId].c_str(), mSamplerNames[samplerId].c_str(),
                        ci(op2).c_str(), ci(op5).c_str(), ci(op6).c_str(), strrchr(op3, '.'));
            else
            {
              int offsetx = 0, offsety = 0, offsetz = 0;
              sscanf_s(statement, "sample_d_aoffimmi_indexable(%d,%d,%d", &offsetx, &offsety,
                       &offsetz);
              sprintf_s(buffer, 512, "  %s = %s.SampleGrad(%s, %s, %s, %s, int2(%d, %d))%s;\n",
                        writeTarget(op1), mTextureNames[textureId].c_str(),
                        mSamplerNames[samplerId].c_str(), ci(op2).c_str(), ci(op5).c_str(),
                        ci(op6).c_str(), offsetx, offsety, strrchr(op3, '.'));
            }
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          ...
          case OPCODE_SAMPLE_C:
          {
            ...
            truncateTexturePos(op2, mTextureType[textureId].c_str());
            if(!instr->bAddressOffset)
              sprintf_s(buffer, 512, "  %s = %s.SampleCmp(%s, %s, %s)%s;\n", writeTarget(op1),
                        mTextureNames[textureId].c_str(), mSamplerComparisonNames[samplerId].c_str(),
                        ci(op2).c_str(), ci(op5).c_str(), strrchr(op3, '.'));
            else
            {
              int offsetx = 0, offsety = 0, offsetz = 0;
              sscanf_s(statement, "sample_c_aoffimmi_indexable(%d,%d,%d", &offsetx, &offsety,
                       &offsetz);
              sprintf_s(buffer, 512, "  %s = %s.SampleCmp(%s, %s, %s, int2(%d, %d))%s;\n",
                        writeTarget(op1), mTextureNames[textureId].c_str(),
                        mSamplerComparisonNames[samplerId].c_str(), ci(op2).c_str(),
                        ci(op5).c_str(), offsetx, offsety, strrchr(op3, '.'));
            }
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          ...
          case OPCODE_SAMPLE_C_LZ:
          {
            ...
            truncateTexturePos(op2, mTextureType[textureId].c_str());
            if(!instr->bAddressOffset)
              sprintf_s(buffer, 512, "  %s = %s.SampleCmpLevelZero(%s, %s, %s)%s;\n",
                        writeTarget(op1), mTextureNames[textureId].c_str(),
                        mSamplerComparisonNames[samplerId].c_str(), ci(op2).c_str(),
                        ci(op5).c_str(), strrchr(op3, '.'));
            else
            {
              int offsetx = 0, offsety = 0, offsetz = 0;
              sscanf_s(statement, "sample_c_lz_aoffimmi_indexable(%d,%d,%d", &offsetx, &offsety,
                       &offsetz);
              sprintf_s(buffer, 512, "  %s = %s.SampleCmpLevelZero(%s, %s, %s, int2(%d, %d))%s;\n",
                        writeTarget(op1), mTextureNames[textureId].c_str(),
                        mSamplerComparisonNames[samplerId].c_str(), ci(op2).c_str(),
                        ci(op5).c_str(), offsetx, offsety, strrchr(op3, '.'));
            }
          ...
          case OPCODE_SAMPLE_POS:
          {
            ...
            sprintf_s(buffer, 512, "  %s = %s.GetSamplePosition(%s);\n", writeTarget(op1),
          ...
          case OPCODE_LOD:
          {
            ...
            if(*clamped == 'x')
              sprintf_s(buffer, 512, "  %s = %s.CalculateLevelOfDetail(%s, %s);\n",
                        writeTarget(op1), mTextureNames[textureId].c_str(),
                        mSamplerNames[samplerId].c_str(), ci(op2).c_str());
            else
              sprintf_s(buffer, 512, "  %s = %s.CalculateLevelOfDetailUnclamped(%s, %s);\n",
                        writeTarget(op1), mTextureNames[textureId].c_str(),
                        mSamplerNames[samplerId].c_str(), ci(op2).c_str());
            appendOutput(buffer);
            removeBoolean(op1);
          ...
          case OPCODE_GATHER4:
          {
            ...
            truncateTexturePos(op2, mTextureType[textureId].c_str());
            if(!instr->bAddressOffset)
              sprintf_s(buffer, 512, "  %s = %s.Gather(%s, %s)%s;\n", writeTarget(op1),
                        mTextureNames[textureId].c_str(), mSamplerNames[samplerId].c_str(),
                        ci(op2).c_str(), strrchr(op3, '.'));
            else
            {
              int offsetx = 0, offsety = 0, offsetz = 0;
              sscanf_s(statement, "gather4_aoffimmi_indexable(%d,%d,%d", &offsetx, &offsety,
                       &offsetz);
              sprintf_s(buffer, 512, "  %s = %s.Gather(%s, %s, int2(%d, %d))%s;\n", writeTarget(op1),
                        mTextureNames[textureId].c_str(), mSamplerNames[samplerId].c_str(),
                        ci(op2).c_str(), offsetx, offsety, strrchr(op3, '.'));
            }
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          ...
          case OPCODE_GATHER4_C:
          {
            ...
            truncateTexturePos(op2, mTextureType[textureId].c_str());
            if(!instr->bAddressOffset)
              sprintf_s(buffer, 512, "  %s = %s.GatherCmp(%s, %s, %s)%s;\n", writeTarget(op1),
                        mTextureNames[textureId].c_str(), mSamplerComparisonNames[samplerId].c_str(),
                        ci(op2).c_str(), ci(op5).c_str(), strrchr(op3, '.'));
            else
            {
              int offsetx = 0, offsety = 0, offsetz = 0;
              sscanf_s(statement, "gather4_c_aoffimmi_indexable(%d,%d,%d", &offsetx, &offsety,
                       &offsetz);
              sprintf_s(buffer, 512, "  %s = %s.GatherCmp(%s, %s, %s, int2(%d,%d))%s;\n",
                        writeTarget(op1), mTextureNames[textureId].c_str(),
                        mSamplerComparisonNames[samplerId].c_str(), ci(op2).c_str(),
                        ci(op5).c_str(), offsetx, offsety, strrchr(op3, '.'));
            }
          ...
          //   srcSampler[.select_component] output.color	= texture2d.Gather(samplerState, input.texcoord, int2(0,0));
          case OPCODE_GATHER4_PO:
          {
            ...
            truncateTexturePos(op2, mTextureType[textureId].c_str());
            sprintf_s(buffer, 512, "  %s = %s.Gather(%s, %s, %s)%s;\n", writeTarget(op1),
                      mTextureNames[textureId].c_str(), mSamplerNames[samplerId].c_str(),
                      ci(op2).c_str(), ci(op3).c_str(), strrchr(op4, '.'));
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          }

          // gather4_po_c dest[.mask], srcAddress[.swizzle], srcOffset[.swizzle],
          // srcResource[.swizzle], srcSampler[.R], srcReferenceValue
          case OPCODE_GATHER4_PO_C:
          {
            ...
            sscanf_s(op5, "s%d", &samplerId);
            truncateTexturePos(op2, mTextureType[textureId].c_str());
            sprintf_s(buffer, 512, "  %s = %s.GatherCmp(%s, %s, %s, %s)%s;\n", writeTarget(op1),
                      mTextureNames[textureId].c_str(), mSamplerComparisonNames[samplerId].c_str(),
                      ci(op2).c_str(), ci(op6).c_str(), ci(op3).c_str(), strrchr(op4, '.'));
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          }
          ...
          case OPCODE_LD:
          {
            ...
            else
            {
              ...
              sprintf_s(buffer, 512, "  %s = %s.Load(%s, int3(%d, %d, %d))%s;\n", writeTarget(op1),
                        mTextureNames[textureId].c_str(), ci(op2).c_str(), offsetU, offsetV,
                        offsetW, strrchr(op3, '.'));
            }
            appendOutput(buffer);
            removeBoolean(op1);
          ...
          case OPCODE_LD_MS:
          {
            ...
            truncateTextureLoadPos(op2, mTextureType[textureId].c_str());
            if(!instr->bAddressOffset)
              sprintf_s(buffer, 512, "  %s = %s.Load(%s, %s)%s;\n", writeTarget(op1),
                        mTextureNames[textureId].c_str(), ci(op2).c_str(), ci(op4).c_str(),
                        strrchr(op3, '.'));
            else
            {
              int offsetU = 0, offsetV = 0, offsetW = 0;
              sscanf_s(statement, "ld_aoffimmi(%d,%d,%d", &offsetU, &offsetV, &offsetW);
              sprintf_s(buffer, 512, "  %s = %s.Load(%s, %s, int3(%d, %d, %d))%s;\n",
                        writeTarget(op1), mTextureNames[textureId].c_str(), ci(op2).c_str(),
                        ci(op4).c_str(), offsetU, offsetV, offsetW, strrchr(op3, '.'));
            }
            appendOutput(buffer);
            removeBoolean(op1);
            break;
            ...
            // ld_structured_indexable(structured_buffer, stride=32)(mixed,mixed,mixed,mixed)
            // r3.xyzw, v0.x, l(16), t0.xyzw
            ...
            // Since this has no prior code, and the text based parser fails on this complicated
            // command, we are switching to using the structure from the James-Jones decoder.
          ...
          case OPCODE_STORE_STRUCTURED:
          {
            ...
            sprintf_s(buffer, 512, "  %s[%s].%s = %s;\n", mTextureNames[textureId].c_str(),
                      ci(op2).c_str(), ci(op3).c_str(), writeTarget(op1));
            appendOutput(buffer);
            break;
          }

            // Missing opcodes for SM5.  Not implemetned yet, but we want to generate some sort of
            // code, in case these are used in needed shaders.  That way we can hand edit the shader
            // to make it usable, until this is completed.
          case OPCODE_STORE_UAV_TYPED:
          case OPCODE_LD_UAV_TYPED:
          case OPCODE_LD_RAW:
          case OPCODE_STORE_RAW:
          {
            sprintf_s(buffer, 512, "// No code for instruction (needs manual fix):\n");
            appendOutput(buffer);
            const char *eolPos = strchr(c + pos, '\n');
            ptrdiff_t len = eolPos - (c + pos);
            std::string line(c + pos, len);
          ...
          case OPCODE_DISCARD:
            applySwizzle(".x", op1);
            if(instr->eBooleanTestType == INSTRUCTION_TEST_ZERO)
              sprintf_s(buffer, 512, "  if (%s == 0) discard;\n", ci(op1).c_str());
            else
              sprintf_s(buffer, 512, "  if (%s != 0) discard;\n", ci(op1).c_str());
            appendOutput(buffer);
            break;
          ...
          case OPCODE_RESINFO:
          {
               ...
               texture.eType == OPERAND_TYPE_RESOURCE)
            {
              ...
              {
                if(returnType == RESINFO_INSTRUCTION_RETURN_UINT)
                  sprintf_s(buffer, 512, "  %s.GetDimensions(0, uiDest.x, uiDest.y, uiDest.z);\n",
                            bindInfo->Name.c_str());
                else
                  sprintf_s(buffer, 512, "  %s.GetDimensions(0, fDest.x, fDest.y, fDest.z);\n",
                            bindInfo->Name.c_str());
                appendOutput(buffer);
              ...
              {
                if(returnType == RESINFO_INSTRUCTION_RETURN_UINT)
                  sprintf_s(buffer, 512, "  %s.GetDimensions(uiDest.x, uiDest.y, uiDest.z);\n",
                            bindInfo->Name.c_str());
                else
                  sprintf_s(buffer, 512, "  %s.GetDimensions(fDest.x, fDest.y, fDest.z);\n",
                            bindInfo->Name.c_str());
                appendOutput(buffer);
                unknownVariant = false;
              }
              else if(bindInfo->eDimension == REFLECT_RESOURCE_DIMENSION_TEXTURE2DARRAY)
              {
                if(returnType == RESINFO_INSTRUCTION_RETURN_UINT)
                  sprintf_s(buffer, 512,
                            "  %s.GetDimensions(0, uiDest.x, uiDest.y, uiDest.z, uiDest.w);\n",
                            bindInfo->Name.c_str());
                else
                  sprintf_s(buffer, 512,
                            "  %s.GetDimensions(0, fDest.x, fDest.y, fDest.z, fDest.w);\n",
                            bindInfo->Name.c_str());
                appendOutput(buffer);
                unknownVariant = false;
              }
              ...
              // Not sure this is fully correct, but the goal here is to apply the swizzle from the
              // op3, which is the texture register, as the pieces that are valid to copy to the op1
              // output. The op3 texture swizzle can determine which components to use, and what
              // order. The op1 output determines which ones are valid for output.
              if(returnType == RESINFO_INSTRUCTION_RETURN_UINT)
              {
            ...
            if(unknownVariant)
            {
              ...
              sprintf_s(buffer, 512, "  Unknown use of GetDimensions for _resinfo: %s\n",
                        line.c_str());
              appendOutput(buffer);
          ...
          case OPCODE_EVAL_SAMPLE_INDEX:
            ...
            sprintf_s(buffer, 512, "  %s = EvaluateAttributeAtSample(%s, %s);\n", writeTarget(op1),
                      op2, op3);
            appendOutput(buffer);
            break;
          case OPCODE_EVAL_CENTROID:
            remapTarget(op1);
            applySwizzle(op1, op2);
            sprintf_s(buffer, 512, "  %s = EvaluateAttributeCentroid(%s);\n", writeTarget(op1), op2);
            appendOutput(buffer);
            break;
          case OPCODE_EVAL_SNAPPED:
            ...
            sprintf_s(buffer, 512, "  %s = EvaluateAttributeSnapped(%s, %s);\n", writeTarget(op1),
                      op2, op3);
            appendOutput(buffer);
            break;
          ...
          case OPCODE_DERIV_RTX_COARSE:
            remapTarget(op1);
            applySwizzle(op1, op2);
            sprintf_s(buffer, 512, "  %s = ddx_coarse(%s);\n", writeTarget(op1), op2);
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          case OPCODE_DERIV_RTX_FINE:
            remapTarget(op1);
            applySwizzle(op1, op2);
            sprintf_s(buffer, 512, "  %s = ddx_fine(%s);\n", writeTarget(op1), op2);
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          case OPCODE_DERIV_RTY_COARSE:
            remapTarget(op1);
            applySwizzle(op1, op2);
            sprintf_s(buffer, 512, "  %s = ddy_coarse(%s);\n", writeTarget(op1), op2);
            appendOutput(buffer);
            removeBoolean(op1);
            break;
          case OPCODE_DERIV_RTY_FINE:
            remapTarget(op1);
            applySwizzle(op1, op2);
            sprintf_s(buffer, 512, "  %s = ddy_fine(%s);\n", writeTarget(op1), op2);
            appendOutput(buffer);
            removeBoolean(op1);
            break;
...
#pragma warning(pop)
  ...
  void WriteAddOnDeclarations()
  {
    ...
    const char *IniTextureCode = "Texture1D<float4> IniParams : register(t120);\n";
...
bytebuf Disassemble(const byte *dxbcCode, size_t size)
{
  ...
  pD3DDisassemble disassembleFunc = (pD3DDisassemble)GetProcAddress(d3dcompiler, "D3DDisassemble");

  if(disassembleFunc == NULL)
  {
  ...
  ID3DBlob *asmBlob = NULL;

  hr = disassembleFunc(dxbcCode, size, 0, NULL, &asmBlob);

  if(hr != S_OK)
    return {};

  bytebuf asmBuf;
  asmBuf.assign((const byte *)asmBlob->GetBufferPointer(), asmBlob->GetBufferSize());

  SAFE_RELEASE(asmBlob);
