...
#pragma warning(disable: 6328)
	...
	void ParseInputSignature(const char *c, size_t size)
	{
		...
		while (pos < size)
		{
			...
			int numRead = sscanf_s(c + pos, "// %s %d %s %d %s %s",
				name, 256, &index, mask, 16, &slot, format2, 16, format, 16);
			if (numRead != 6)
			{
	...
	void ParseOutputSignature(const char *c, size_t size)
	{
		...
		while (pos < size)
		{
			...
			{
				char sysValue[64];
				sscanf_s(c + pos, "// %s %d %s %s %s %s",
					name, 256, &index, mask, 16, sysValue, 64, format2, 16, format, 16);
				// Write.
				char buffer[256];
	...
	void ReadResourceBindings(const char *c, size_t size)
	{
		...
		while (pos < size)
		{
			...
			int numRead = sscanf_s(c + pos, "// %s %s %s %s %d %d",
				name, 256, type, 16, format, 16, dim, 16, &slot, &arraySize);
			if (numRead != 6)
					...
					else
					{
						...
						if (useInt)
							...
							numRead = sscanf_s(c + pos, "// = 0x%lx 0x%lx 0x%lx 0x%lx;", (unsigned long *)(v + 0), 
								(unsigned long *)(v + 1), (unsigned long *)(v + 2), (unsigned long *)(v + 3));

						if (structLevel < 0)
						{
							if (suboffset == 0)
								sprintf_s(buffer, 256, "  %s%s %s : packoffset(c%d) = %s(", modifier.c_str(), type, name, packoffset, type);
							else
								sprintf_s(buffer, 256, "  %s%s %s : packoffset(c%d.%c) = %s(", modifier.c_str(), type, name, packoffset, INDEX_MASK[suboffset], type);
						}
						else
							sprintf_s(buffer, 256, "  %s%s%s %s = %s(", structSpacing.c_str(), modifier.c_str(), type, name, type);
						mOutput.insert(mOutput.end(), buffer, buffer + strlen(buffer));
	...
	char statement[128],
		op1[opcodeSize], op2[opcodeSize], op3[opcodeSize], op4[opcodeSize], op5[opcodeSize], 
		op6[opcodeSize], op7[opcodeSize], op8[opcodeSize], op9[opcodeSize], op10[opcodeSize], 
		op11[opcodeSize], op12[opcodeSize], op13[opcodeSize], op14[opcodeSize], op15[opcodeSize];
	...
	char *convertToInt(char *target)
	{
		...
		if (!strncmp(target + isMinus, "float", 5))
		{
			...
			if (size == 2)
				sprintf_s(buffer + strlen(buffer), opcodeSize - strlen(buffer), "int2(%d,%d)", (int)f0, (int)f1);
			else if (size == 3)
				sprintf_s(buffer + strlen(buffer), opcodeSize - strlen(buffer), "int3(%d,%d,%d)", (int)f0, (int)f1, (int)f2);
			else if (size == 4)
				sprintf_s(buffer + strlen(buffer), opcodeSize - strlen(buffer), "int4(%d,%d,%d,%d)", (int)f0, (int)f1, (int)f2, (int)f3);
	...
	char *convertToUInt(char *target)
	{
		...
		if (!strncmp(target + isMinus, "float", 5))
		{
			...
			if (size == 2)
				sprintf_s(buffer + strlen(buffer), opcodeSize - strlen(buffer), "uint2(%d,%d)", (int)f0, (int)f1);
			else if (size == 3)
				sprintf_s(buffer + strlen(buffer), opcodeSize - strlen(buffer), "uint3(%d,%d,%d)", (int)f0, (int)f1, (int)f2);
			else if (size == 4)
				sprintf_s(buffer + strlen(buffer), opcodeSize - strlen(buffer), "uint4(%d,%d,%d,%d)", (int)f0, (int)f1, (int)f2, (int)f3);
			...
			if (depthTexture != mTextureNames.end())
			{
				...
				if (!ZRepair_Dependencies1.size() || found == (1 << ZRepair_Dependencies1.size()) - 1)
				{
					...
					while (pos)
					{
						...
						if (!wposAvailable)
						{
							sprintf_s(buf, 512, "float4 zpos4 = %s;\n"
								"float zTex = zpos4.%c;\n"
								"float zpos = %s;\n"
				...
				if (depthTexture != mTextureNames.end())
				{
					...
					if (!ZRepair_Dependencies2.size() || found == (1 << ZRepair_Dependencies2.size()) - 1)
					{
						...
						if (pos)
						{
							...
							char buf[256];
							sprintf_s(buf, 256, "float4 zpos4 = %s;\n"
								"float zTex = zpos4.%c;\n"
								"float zpos = %s;\n"
			...
			else if (!strcmp(statement, "dcl_constantbuffer"))
			{
				...
				if (strPos)
				{
					...
					if (i == mCBufferData.end())
					{
						...
						e.matrixRow = 0;
						e.isRowMajor = false;
						sprintf_s(buffer, 512, "cbuffer cb%d : register(b%d)\n"
							"{\n"
							"  float4 cb%d[%d];\n"
				...
				switch (instr->eOpcode)
				{
					...
					case OPCODE_UBFE:
					{
						...
						while (*++pop1)
						{
							...
							appendOutput(buffer);
							sprintf_s(buffer, 512, "%s = (int)%s << (32-(%s + %s)); %s = (uint)%s >> (32-%s); ", writeTarget(op5), ci(GetSuffix(op4, idx)).c_str(), ci(GetSuffix(op2, idx)).c_str(), ci(GetSuffix(op3, idx)).c_str(), writeTarget(op5), writeTarget(op5), ci(GetSuffix(op2, idx)).c_str());
							appendOutput(buffer);
					...
					case OPCODE_BFI:
					{
						...
						while (*++pop1)
						{
							...
							sprintf_s(buffer, 512, "  bitmask.%c = ((~(-1 << %s)) << %s) & 0xffffffff;\n"
					...
					case OPCODE_LD_STRUCTURED:
					{
						...
						for (int component = 0; component < 4; component++)
						{
							if (instr->asOperands[0].ui32CompMask & (1 << component))
							{
								...
								sprintf_s(buffer, 512, "%s.%s = StructuredBufferName[srcAddressRegister].srcByteOffsetName.swiz;\n", dst0.c_str(), swiz.c_str());
								appendOutput(buffer);
							}
						}
						removeBoolean(op1);
					...
					case OPCODE_RESINFO:
					{
							...
							&& texture.eType == OPERAND_TYPE_RESOURCE)
						{
							...
							{
								if (returnType == RESINFO_INSTRUCTION_RETURN_UINT)
									sprintf_s(buffer, 512, "  %s.GetDimensions(0, uiDest.x, uiDest.y, uiDest.z, uiDest.w);\n", bindInfo->Name.c_str());
								else
									sprintf_s(buffer, 512, "  %s.GetDimensions(0, fDest.x, fDest.y, fDest.z, fDest.w);\n", bindInfo->Name.c_str());
								appendOutput(buffer);
								unknownVariant = false;
							}
