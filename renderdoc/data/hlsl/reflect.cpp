...
static const uint32_t* ReadResourceBinding(const uint32_t* pui32FirstResourceToken, const uint32_t* pui32Tokens, ResourceBinding* psBinding)
{
    ...
    psBinding->eType = (ResourceType_TAG) *pui32Tokens++;
    ...
    const uint32_t* pui32FirstConstBufToken, const uint32_t* pui32Tokens, ConstantBuffer* psBuffer)
{
    ...
    for(i=0; i<ui32VarCount; ++i)
    {
        ...
        if(ui32DefaultValueOffset)
        {
			...
			{
				sVar.pui32DefaultValues.push_back(pui32DefaultValToken[j]);
			}
        }
                   ...
                   ShaderInfo* psShaderInfo)//out
{
    ...
    // D3DCompile flags? http://msdn.microsoft.com/en-us/library/gg615083(v=vs.85).aspx
    //uint32_t ui32CompileFlags = *pui32Tokens++;
...
int GetResourceFromBindingPoint(ResourceType_TAG eType, uint32_t ui32BindPoint, ShaderInfo* psShaderInfo, ResourceBinding** ppsOutBinding)
{
    uint32_t i;
